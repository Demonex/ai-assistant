...
export const usePreventAutofill = <T = HTMLInputElement>({
	...
	const handleFocus = useCallback<React.FocusEventHandler<T>>(
		(e) => {
			setReadOnly(false);
			onFocus?.(e);
		},
	...
	const handleTouchStart = useCallback<React.TouchEventHandler<T>>(
		(e) => {
			setReadOnly(false);
			onTouchStart?.(e);
		},
	...
	const handleContextMenu = useCallback<React.MouseEventHandler<T>>(
		(e) => {
			e.preventDefault();
			e.stopPropagation();
			onContextMenu?.(e);
