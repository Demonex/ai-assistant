...
export const _useFetch = <T = any>({
	onFailure,
	onSuccess,
	...props
}: ApiAction<T> & Record<string, unknown>) => {
...
export const useFetch = (args) =>
	useSharedHook<ReturnType<typeof _useFetch>>(_useFetch, args);
...
export const _useLazyFetch = <T = any>(
	...
	ReduceProps<T>,
	(
		lazyProps?: ApiAction<T> & Record<string, unknown>,
	) => Promise<FetchResponse<T>>,
] => {
	let unmount = false;
	const [state, dispatch] = React.useReducer(
...
export const useLazyFetch = <T = any>({
	...
	method,
	cache,
}: { url?: string; method?: string; cache?: boolean } = {}) => {
	return useBetween<ReturnType<typeof _useLazyFetch<T>>>(
		lazyMemo(url, method, cache),
	);
};
