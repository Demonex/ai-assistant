			...
			<div className="flex flex-col gap-16">
				<div className="flex flex-col gap-6 items-center">
					...
					<div className="flex flex-col gap-4 items-center">
						...
						<p className="text-caption_m_desk text-medium_grey flex items-center gap-1 whitespace-nowrap  max-w-[12rem]">
				...
				<div>
					<div
						className="py-4 flex gap-4 border-b border-[#33333380] items-center cursor-pointer"
						onClick={() => setAccountSettingsType("account")}
					>
						<AccountIcon
							className={`${accountSettingsType === "account" ? "fill-medium_grey" : "fill-white"}`}
						/>
						<p
							className={`text-btnText ${accountSettingsType === "account" ? "text-medium_grey" : "text-white"} `}
						>
							Аккаунт
						</p>
					</div>
					<div
						className="py-4 flex gap-4 items-center cursor-pointer"
						onClick={() => setAccountSettingsType("settings")}
					>
						<SettingsIcon
							className={`${accountSettingsType === "settings" ? "stroke-medium_grey" : "stroke-white"}`}
						/>
						<p
							className={`text-btnText ${accountSettingsType === "settings" ? "text-medium_grey" : "text-white"} `}
						>
							Настройки
						</p>
					</div>
