  ...
  return (
      <Listbox value={selectedSubscription} onChange={setSelectedSubscription}>
        ...
        >
          ...
          >
            {artists?.map(([_, value], index) => value.name !== selectedSubscription.name ? (
              <Listbox.Option
                key={index}
                value={value}
  ...
  return (
    <>
      ...
      <section className='py-6 flex flex-col gap-6'>
        ...
        <ol className='lg:w-1/2' type='1'>
          {
            sources.map((source, index) => (
              <li key={index} className='px-6 py-4 border-b border-secondary_dark_gray flex justify-between'>
                <div className='flex gap-2 items-center'>
        ...
        <p className='text-caption_s_desk text-medium_grey lg:w-1/2 mt-8 lg:mt-[unset]'>*компания Meta Platforms Inc., владеющая Facebook и
