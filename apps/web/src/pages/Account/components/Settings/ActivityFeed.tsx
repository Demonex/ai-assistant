...
const FilterParameters = memo(({currentArtist, filterLevel, setFilterLevel,setOpenMobileFilter}: any) => {
  const filterOptions = useMemo(() => {
    return activityFeedFilter.map((social) => {
      const curr = navbar[0].content[0].options.find(item => item.slug === social.source);
      return {
        ...(social || {}),
        logo: curr?.logo,
      }
  ...
  return (
    <div className='w-full lg:w-1/2 flex flex-col gap-4 md:gap-6 min-w-[22rem]'>
      <div className='flex gap-2 items-center lg:hidden mb-4 md:mb-[unset]' onClick={() => setOpenMobileFilter(false)}>
        <ArrowBack className='fill-white w-5 h-5'/>
        <p className='text-t2Regular'>Все подписки</p>
      </div>
      <h2 className='text-btnText text-light_grey md:mt-10 lg:mt-[unset]'>Настройка фильтрации</h2>
      {
        currentArtist
          ...
          : <p className='text-t2Regular text-medium_grey'>Выбери подписку в списке слева</p>
      }
      ...
      <div className='py-4 flex flex-col md:flex-row md:items-center gap-10 flex-wrap '>
        ...
        <div className='flex gap-2 items-center'>
          <input
            ...
            type='checkbox'
            className='min-w-[1.125rem] h-[1.125rem] border border-solid border-secondary_dark_gray rounded-sm bg-popup_gray'/>
          <p className='text-t2Regular whitespace-nowrap'>Только редакционные</p>
          <InfoIcon className='fill-white'/>
        </div>
        <div className='flex gap-2 items-center'>
          <input
      ...
      <div className="relative w-full lg:max-w-[35rem]">
        ...
        <div className="w-full ">
                      <span
                        className="text-caption_r_desk text-medium_grey dark:text-gray-400 absolute start-0 -bottom-8">Низкий</span>
          <div
            className=" absolute start-0 h-5 w-[1px] bg-medium_grey top-1/2 -translate-y-1/2 -z-10"/>
          <span
            className="text-caption_r_desk text-medium_grey dark:text-gray-400 absolute start-1/3 -bottom-8">Средний</span>
          <div
            className=" absolute start-0 h-5 w-[1px] bg-medium_grey top-1/2 -translate-y-1/2 -z-10"/>
          <span
            className="text-caption_r_desk text-medium_grey dark:text-gray-400 absolute start-2/3 -translate-x-1/4 rtl:translate-x-1/4 -bottom-8">Высокий</span>
          <div
            className=" absolute start-1/2 h-5 w-[1px] bg-medium_grey top-1/2 -translate-y-1/2 -z-10"/>
          <span
            className="text-caption_r_desk text-medium_grey dark:text-gray-400 absolute end-0   -bottom-8 ">Макс.</span>
          <div
            ...
            <ul>
              {
                filterOptions.map((item, index) => (
                  <li key={index} className='py-2 flex gap-2 items-center'>
                    ...
                    <p className='text-caption_r_desk text-medium_grey'>{filterLevel === '1' ? item.baselineDescription
                      : filterLevel === '2' ? item.midRangeDescription
                        : filterLevel === '3' ? item.highRangeDescription
                          : item.topRangeDescription}</p>
                  </li>
                ))
              }
            </ul>
    ...
    <>
      ...
      <section className='w-full py-10 flex justify-between gap-10'>
        <div className='w-full lg:max-w-[35.25rem] flex flex-col gap-6'>
          <h2 className='text-btnText text-light_grey'>Уровень фильтрации по подпискам</h2>
          <ul className=''>
            {
              Object.entries(artistsFromSubscriptions || {}).map(([_, value], index) => {
                return (
                  <li
                    ...
                    <div className='flex gap-4 w-2/3 items-center'>
                      <img src={value.imageUrl} alt='' className='w-11 h-11 rounded-full'/>
                      <p
                        className={`text-caption_r_desk ${currentArtist === value ? 'text-medium_grey' : ''}`}>{value.name}</p>
                    </div>
                    <div className='flex gap-4 w-1/3 justify-between'>
                      <p
                        className={`text-caption_r_desk ${currentArtist === value ? 'text-medium_grey' : 'text-light_grey'}`}>{filterLevel === '1' ? 'Низкий' : filterLevel === '2' ? 'Средний' : filterLevel === '3' ? 'Высокий' : 'Макс.'}</p>
                      <ChevronRight
        ...
        <ShowOnLaptopToDesktop>
          <FilterParameters filterLevel={filterLevel} currentArtist={currentArtist} setFilterLevel={setFilterLevel} setOpenMobileFilter={setOpenMobileFilter}/>
        </ShowOnLaptopToDesktop>
