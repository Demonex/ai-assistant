...
const TableItemMobile = memo<{
	...
	({
		...
		useEffect(() => {
			...
			setSubscriptions((prev) =>
				prev.reduce((prev, next) => {
					return [
						...prev,
						next.id === dataUpdateSubscription.id
							? dataUpdateSubscription
							: next,
					];
				}, []),
		...
		const endDate = DateTime.fromISO(item.createdAt)
			.plus({ month: periodByMonth })
			.toFormat("d/MM/yyyy");
		const nextPayment = DateTime.fromISO(item.createdAt)
			.plus({ month: periodByMonth })
			.minus({ day: 1 })
			.toFormat("d/MM/yyyy");
		return (
			<>
				<Transition appear show={renderItemContent} as={Fragment}>
					<Dialog
						...
						onClose={() => setRenderItemContent(false)}
					>
						<div className="">
							<Transition.Child
							...
							>
								<Dialog.Panel className="w-full bg-[#0C0C0C] py-6 px-4 mt-[4.25rem]">
									<div
										...
										onClick={() => setRenderItemContent(false)}
									>
										...
										<div className="flex gap-2 items-center">
											{isPackage ? (
												<p className={" text-caption_r_desk"}>
													Лейбл до {item.plan.limit} артистов
												</p>
											) : (
												<>
													<img
											...
											<ul className="flex flex-col w-full ">
												{item.artists.map((id, index) => (
													<li
														key={index}
														className={
															"flex items-center gap-4 py-4 px-2 border-b border-b-secondary_dark_gray bg-popup_gray/50"
														}
													>
														<img
															src={artistsFromSubscriptions[id]?.imageUrl}
															alt={artistsFromSubscriptions[id]?.name}
															className="w-11 h-11 rounded-full"
														/>
														<p className="text-t2Regular">
															{artistsFromSubscriptions[id]?.name}
														</p>
													</li>
												))}
												<li
													className={
														"flex w-full items-center gap-4 py-4 pl-4 bg-[#0C0C0C] cursor-pointer border-b border-secondary_dark_gray/50"
													}
													onClick={() => handleAddArtistClick(item.id)}
												>
													<div className="w-11 h-11 rounded-full flex items-center justify-center bg-primary_blue">
														<svg
															width="28"
															height="28"
															viewBox="0 0 28 28"
													...
													<p className="text-caption_m_desk">
														Добавить артиста
													</p>
												</li>
											</ul>
				...
				<div
					className="py-4 flex justify-between items-center  border-b border-secondary_dark_gray/50"
...
const TableItem = memo<{
	...
	const { setSubscriptions, dataUpdateSubscription, fetchUpdateSubscriptions } =
		useSubscriptions();
	const [isRenew, setIsRenew] = useState(item.renew);
		...
		<>
			<td className="hidden md:block lg:hidden p-4">
				<div className="flex gap-4 text-black justify-between">
					{isPackage ? (
						<p
							className={`text-caption_r_desk ${item.archived ? "text-medium_grey" : ""}`}
						>
							Лейбл до {item.plan.limit} артистов
						</p>
					) : (
						<div className="flex gap-4 items-center">
							...
							/>
							<p
								className={` text-caption_r_desk ${item.archived ? "text-medium_grey" : "text-white"}`}
							>
								{artist?.name}
							</p>
						</div>
					...
					<div className="flex gap-6 items-center">
						{!item.archived ? (
							<div className="flex gap-3 items-center">
								...
								/>
								<p
									className={`text-caption_r_desk ${item.plan.limit > 1 ? "text-black" : "text-white"}`}
								>
									Автопродление
								</p>
							</div>
						...
						{!item.archived && (
							<div className="flex gap-4 items-center">
								<ChevronYellowIcon
									className={`cursor-pointer ${item.plan.limit > 1 ? "fill-black" : "fill-yellow"}`}
								/>
								<button
									onClick={(e) => {
										e.preventDefault();
								...
								>
									<BasketIcon
										className={`cursor-pointer ${item.plan.limit > 1 ? "fill-black" : "fill-white"}`}
									/>
								</button>
							</div>
						)}
				...
				<div className="mt-4 flex justify-between">
					<div className="flex flex-col  gap-2">
						<p
							className={` text-caption_r_desk capitalize ${item.archived ? "text-medium_grey" : ""}`}
						>
							Статус
						</p>
						<p
			...
			<ShowOnLaptopToDesktop>
				<>
					...
					<td
						className={`p-4 text-caption_r_desk capitalize ${item.archived ? "text-medium_grey" : ""}`}
					>
						{status}
					</td>
					<td
...
export const Subscriptions = memo(() => {
	...
	useEffect(() => {
		...
		setSubscriptions((prev) =>
			prev.reduce((prev, next) => {
				return [...prev, next.id === dataAddArtist.id ? dataAddArtist : next];
			}, []),
		);
	}, [dataAddArtist]);

	const handleAddArtistClick = useCallback(
		...
		<>
			...
			<div className="py-4">
				...
				<div className="hidden md:block ">
					<div className=" bg-[#272727]/50 lg:rounded-t-[20px] border-b border-b-secondary_dark_gray block overflow-y-auto">
						<table className="w-full ">
							<thead className="hidden lg:contents">
								<tr>
									{subscriptionTitles.map((title, index) => (
										<th
											className="py-5 px-4 text-start  text-btnText whitespace-nowrap"
											key={index}
										>
											{title.title}
										</th>
							...
							<tbody className="divide-y divide-secondary_dark_gray bg-[#0C0C0C]">
									...
									.map((item, index) =>
										item.plan.limit > 1 ? (
											<Disclosure key={index}>
												<Disclosure.Button
													as="tr"
													className="bg-yellow text-black cursor-pointer"
												>
													<TableItem
														item={item}
														artistsFromSubscriptions={artistsFromSubscriptions}
														...
														<ul className="flex flex-col w-full ">
															{item.artists.map((id, index) => (
																<li
																	key={index}
																	className={
																		"flex items-center gap-4 py-4 pl-4 border-b border-b-secondary_dark_gray bg-popup_gray/50"
																	}
																>
																	<img
																		src={artistsFromSubscriptions[id]?.imageUrl}
															...
															<li
																className={
																	"flex items-center gap-4 py-4 pl-4 bg-[#0C0C0C] w-fit cursor-pointer"
																}
																onClick={() => handleAddArtistClick(item.id)}
															>
																<div className="w-11 h-11 rounded-full flex items-center justify-center bg-primary_blue">
								...
								{subscriptions?.filter((item) => item.archived).length > 0 && (
									<Disclosure>
										<Disclosure.Button
											as="tr"
											className="bg-medium_grey text-black cursor-pointer"
										>
											<td className="p-4 text-start flex items-center gap-2.5 min-w-[20rem] text-t2Regular">
												<p>Архив</p>
											</td>
											...
											.map((item, index) => (
												...
												>
													<TableItem
														...
														onRemoveFromArchive={onRemoveFromArchive}
													/>
													<td className={"p-4 hidden lg:block"} colSpan={3}>
														<button
															onClick={(e) => {
																e.preventDefault();
																onRemoveFromArchive(item);
