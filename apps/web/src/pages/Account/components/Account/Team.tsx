...
const subscriptions = [{ name: "Dua Lipa" }, { name: "Morgenstern" }];
const Team = () => {
		...
		<>
			...
			<section className="py-6 flex flex-col gap-6">
					...
					>
						...
						>
							<Listbox.Options className="bg-popup_gray  rounded-b-xl  max-w-[35rem] absolute w-full">
								{subscriptions.map((source, index) =>
									source.name !== selectedSubscription.name ? (
										<Listbox.Option
											key={index}
											value={source}
						...
						<tbody className="divide-y divide-secondary_dark_gray">
							{teamMembers.map((item, indexItem) => (
								<tr className={""} key={indexItem}>
									<td className={"p-4 text-caption_r_desk text-light_grey"}>
										{item.name}
									</td>
									...
									<td className={" text-caption_r_desk text-light_grey "}>
										{item.type === "member" && (
											<div
												className="flex justify-end pr-8 items-center"
												onClick={() => setOpenModalRemove(true)}
											>
												<BasketIcon className="fill-white hover:fill-medium_grey cursor-pointer" />
											</div>
										)}
									</td>
								</tr>
				...
				<SecondaryButton
					...
					className="border-none bg-primary_blue text-white w-fit hidden md:flex"
				...
				<div className="md:hidden">
					{teamMembers.map((member, index) => (
						<div
							key={index}
							className="py-4 flex flex-col gap-6 border-b border-secondary_dark_gray/50"
						>
							...
							{member.type === "member" && (
								...
								>
									<BasketIcon className="fill-white hover:fill-medium_grey cursor-pointer w-5 h-5" />
								</SecondaryButton>
							)}
						</div>
					))}
				...
				<SecondaryButton
					title="Добавить участника"
					className="border-none bg-primary_blue text-white w-full md:hidden"
					onClick={() => setOpenModalAdd(true)}
				/>
			</section>
		</>
