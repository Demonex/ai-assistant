	...
	return (
		<>
			...
			<section className="py-8 flex flex-col gap-6">
				...
				<div>
					<p className="text-t2Regular text-light_grey py-1">
						{" "}
						Сумма в месяц:2480 ₽{" "}
					</p>
					<p className="text-t2Regular text-light_grey py-1">
						{" "}
						Дата следующего списания: 22.10.2024{" "}
					</p>
					<p className="text-t2Regular text-light_grey py-1">
						{" "}
						Сумма следующего списания: 690 ₽
					</p>
					<p className="text-t2Regular text-light_grey py-1">
						{" "}
						Способ оплаты: Robocassa{" "}
						<span className="cursor-pointer hover:text-medium_grey text-white text-btnText">
							Изменить
						</span>
					</p>
				</div>
				<div>
					<div className="px-6 py-4 flex gap-4 rounded-xl border border-secondary_dark_gray w-fit items-center">
						...
						<div>
							<p className="text-btnText">
								Переходи на тариф “Лейбл” и получай выгоду
							</p>
							<p className="text-t2Regular">
								от 2330 ₽{" "}
								<span className="text-light_grey">в месяц за 5 артистов</span>
							</p>
						</div>
					</div>
					<SecondaryButton
						title="Перейти на другой тариф"
						className="bg-primary_blue border-none text-white mt-5"
							...
							{subscriptions.map((item, index) => (
								...
								>
									...
									<div className="flex gap-4 justify-between items-center">
										...
										<DownloadIcon className="fill-white cursor-pointer hover:fill-medium_grey" />
									</div>
								</li>
							))}
						</ul>
								...
								<tbody className="divide-y divide-secondary_dark_gray">
									{subscriptions.map((item, indexItem) => (
										<tr className={""} key={indexItem}>
											<td className={"p-4 text-caption_r_desk "}>
												{item.prolongDate}
											</td>
											<td className="p-4 text-start flex items-center gap-2.5 min-w-[15rem] text-t2Regular">
												{item.name}
											</td>
											<td className={"p-4 text-caption_r_desk "}>{item.sum}</td>
											<td className={"p-4 text-caption_r_desk "}>
												<DownloadIcon className="fill-white cursor-pointer hover:fill-medium_grey" />
											</td>
										</tr>
