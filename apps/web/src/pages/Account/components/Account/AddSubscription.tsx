...
const AddSubscription = memo(() => {
	const { openAddSubscription, setOpenAddSubscription, setOpenModal } =
		...
		<Transition appear show={openAddSubscription} as={Fragment}>
			...
			>
				...
				<div className="">
					...
					>
						<Dialog.Panel className=" bg-popup_gray lg:my-20 p-4 md:py-8 lg:py-10 md:px-8 lg:px-[3.75rem] lg:mx-auto lg:rounded-[20px] z-50 relative w-full lg:w-fit max-w-[53.7rem]">
							<div
							...
							<div className="flex flex-col gap-6">
								<div className="flex flex-col gap-4">
									...
									<p className="text-caption_r_desk text-medium_grey">
										У тебя есть свободные слоты в действующих подписках. Можем
										воспользоваться одним из них либо оформить новую подписку.
									</p>
								</div>
								<div className="flex flex-col gap-4 ">
										...
										<table className="border-b border-secondary_dark_gray">
											<thead>
												<tr className="bg-[#48484840]/25">
													...
													{subscriptionTitles.map((title, index) => (
														<th
															className="py-5 px-4 text-start  text-btnText"
															key={index}
														>
															{title.title}
														</th>
											...
											<tbody className="divide-y divide-secondary_dark_gray">
												{subscriptions.map((item, indexItem) => (
													<tr className={""} key={indexItem}>
														<td className={"p-4 text-caption_r_desk "}>
															<input
																type="radio"
																className="bg-popup_gray border border-solid border-secondary_dark_gray"
																name="subscriptions"
															/>
														</td>
												...
												<tr>
													...
													<td className={"p-4 text-t2Regular "}>
														Новая подписка
													</td>
												</tr>
											</tbody>
							...
							<div className="mt-6 flex flex-col gap-6">
								<div className="w-full flex justify-between gap-3.5">
									...
									<SecondaryButton
										title="Продолжить"
										className="w-full bg-medium_grey text-white border-none"
										onClick={() => {
