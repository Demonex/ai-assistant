...
const _useFavoriteSources = (): {
	...
	const [favoriteSources, setFavoriteSources] = useState<FavoriteSourcesType>(
		JSON.parse(localStorage.getItem("favoriteSource") || "[]"),
	);
	...
	const removeFavoriteSources = (slug: string) => {
		const filteredFavoriteSources = favoriteSources.filter(
			(item) => item !== slug,
		);
...
export const useFavoriteSources = () =>
	useSharedHook<ReturnType<typeof _useFavoriteSources>>(_useFavoriteSources);
