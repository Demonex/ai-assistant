...
const _useSubscriptions = () => {
	...
	const [{ data: dataUpdateSubscription }, fetchUpdateSubscriptions] =
		useLazyFetch({
			url: `${BACKEND_URL}/subscription/:id/update`,
			method: "put",
			cache: false,
		});
	const [{ data: userSubscriptions, loading, error }, fetchSubscriptions] =
		useLazyFetch({
	...
	const artistsIds = useMemo(() => {
		return (
			subscriptions?.reduce((prev, next) => {
	...
	useEffect(() => {
		...
		artistsIds.forEach((id) => {
			fetchArtist({
				...
				.then(({ data }) => {
					setArtistsFromSubscriptions((prev) => ({
						...prev,
						[id]: data?.account,
					}));
				})
...
export const useSubscriptions = () =>
	useSharedHook<ReturnType<typeof _useSubscriptions>>(_useSubscriptions);
