...
const LilacCTACustomers = memo(() => {
	const { elementRange, isMobile, isTablet } = useSizes();
	const { elementRange: elementRangeLaptop } = useSizes(1024, 1920);
	const { h1SizeMobile, h1Size, paddingHorizontal, marginVertical } =
		useElementRangeSize();
	const { elementRange: elementRangeMobile } = useSizes(320, 1024);
	const horizontalPadding = elementRangeLaptop(30, 77);
	const verticalPadding = elementRange(20, 152);
	const imgSize = elementRange(230, 430);
	const imgSizeMob = elementRangeMobile(360, 670);
	const sparySize = elementRangeLaptop(200, 300);
	const sparySizeMobile = elementRangeMobile(130, 200);
			...
			>
				<div className="w-full lg:w-[60%] flex flex-col items-center gap-8 lg:block">
					<div className="w-full 2xl:w-full">
						<h1
							className=" text-h2Desctop font-bold lg:font-black text-center lg:text-start"
							style={{
								fontSize:
						...
						>
							...
							<span className="z-20 relative ">
								впереди!
								<img
									className="absolute -z-10 -top-[20%] -right-3"
									style={{
										minWidth:
											isMobile || isTablet
												? `${sparySizeMobile}px`
...
export default LilacCTACustomers;
