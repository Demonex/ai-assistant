...
const TableItem = memo<{
	...
	return value.cells.map(
		(cell, index) =>
			cell.displayText && (
				<td
					className={`p-4  ${index === 0 ? "pl-8 " : ""}  ${index === rowsLength - 1 ? "pr-8" : ""}`}
					key={index}
				>
					<div className="flex gap-4 items-center">
						...
						<p
							className={`text-caption_r_desk truncate ${index === 0 ? "text-white" : "text-light_grey"}`}
						>
							{cell.displayText}
						</p>
					</div>
...
const TrackTable = memo(() => {
	...
	const rows: (typeof tableData)["tableData"][number]["data"]["rows"][number][] =
		tableData.tableData[dataTrackType].data.rows.filter((item) => {
			if (!searchValue) {
				return true;
			}
			return item.cells.filter((val) => {
				return String(val.displayText)
					.toLowerCase()
			...
			<div className="w-full flex justify-between">
				<div className="flex gap-2 items-center">
					<button onClick={clickBack}>
						<ChevronRight
							className={` w-5 h-5 rotate-180 ${page === 1 ? "fill-medium_grey" : "fill-white"}`}
						/>
					</button>
					<div className="py-2 px-4 text-caption_r_desk text-medium_grey">
						<span>
							{page}/{chunkedRows.length}
						</span>
					</div>
					<button onClick={clickNext}>
						<ChevronRight
							className={` w-5 h-5 ${page === chunkedRows.length ? "fill-medium_grey" : "fill-white"}`}
						/>
					</button>
				</div>
				<div className="flex items-center gap-10 w-1/2 justify-end">
					<div className="hidden md:flex py-2.5 px-3.5 border border-solid border-secondary_dark_gray rounded-xl gap-2 items-center w-full max-w-[20rem]">
					...
					<table className=" w-full">
						<thead>
							<tr className="bg-[#27272780]">
								{titles
									.filter((_, index) => index !== titles.length - 1)
									.map((title, index) => (
										<th
