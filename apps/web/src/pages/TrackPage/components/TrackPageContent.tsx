	...
	return (
		<div className="flex lg:pl-[15.25rem]  lg:px-4 overflow-x-hidden flex-col items-center w-full relative  lg:mb-[unset] mt-[4.25rem] md:mt-[5.25rem] lg:mt-[6rem]">
			<main className="h-full relative z-20 pt-6 w-full px-4 md:px-6 lg:pl-6 lg:pr-2 flex flex-col gap-4 ">
				<header
					id="header"
					className="flex flex-col gap-4 lg:flex-row items-start pb-4 "
				>
					<div className="flex w-full items-center gap-4 md:gap-6 ">
						...
						<div className="flex flex-col items-start w-full  gap-2 ">
							<h1 className="text-[1.25rem] font-medium md:text-t1Semi_deck  md:max-w-[unset]">
								{trackData?.trackInfo?.trackName}
							</h1>
							<p className="text-t2Regular text-light_grey">
								{trackData?.trackInfo?.artistName}
							</p>
						</div>
					</div>
					<div className="flex w-full justify-start md:justify-end lg:items-center gap-8">
						<div className="relative ">
							<Popover>
								{({ open }) => (
									<>
										<Popover.Button>
											<div className="w-7 h-7 border border-light_grey rounded-lg flex justify-center items-center text-caption_s_desk cursor-pointer">
										...
										>
											<Popover.Panel static>
												<div className="absolute bg-popup_gray px-4 top-[calc(100%+1rem)] -left-[8rem] z-50  rounded-[4px] max-w-[24.5rem] max-h-[27.5rem] overflow-y-scroll transition-all opacity-100">
													<div className="overflow-y-auto">
														<ul className="py-4">
															{getCurrentSourceLinks?.map((link, index) => (
																<li
																	className="py-3.5 px-5 border-b border-secondary_dark_gray"
																	key={index}
																>
																	<a
																		href={link.url}
						...
						<Link
							to={`/share/track/${trackId}/${trackData?.trackInfo?.baseUrl.split("/").pop()}`}
						>
				...
				<section className="h-full mb-16 relative">
					<div className="h-full relative z-10">
						<div className="h-full flex  mb-6 flex-col overflow-y-scroll pb-[100px] overflow-x-hidden md:overflow-x-[unset]">
							...
							<p className="text-[10px] leading-3 text-medium_grey mt-2 lg:hidden">
