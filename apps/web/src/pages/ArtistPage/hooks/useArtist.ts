...
const _useArtist = (): {
	...
	const setSource = useCallback(
		(source: string) => {
			const sourceFound = favoriteSources.find(
				({ slug }) => slug === source,
			)?.slug;
			if (!sourceFound) {
				return;
	...
	const setIdFromTrack = useCallback(
		(id: string) => {
			setParams((params) => ({
				...params,
				id,
...
export const useArtist = () =>
	useSharedHook<ReturnType<typeof _useArtist>>(_useArtist);
