		...
		<div className="w-full border-t border-slate-600/40 mt-20 flex flex-col items-center">
			...
			<div className="w-full py-4 ">
				<div className="overflow-x-auto">
					<div className="inline-block w-full align-middle border border-gray-800 rounded-xl overflow-hidden">
						<table className="min-w-full divide-y divide-gray-700 w-full">
							<thead>
								<tr>
									{Array.from({ length: 4 }).map(
										(headerTitle, indexHeaderTitle) => (
											<th
												scope="col"
												className={
													" p-5 text-left uppercase text-xs font-medium cursor-pointer basis-1 "
												}
									...
									return (
										<tr key={i}>
											...
											{Array.from({ length: 3 }).map((data, indexData) => {
												return (
													<td
														className="whitespace-nowrap p-5 text-xs text-gray-300"
														key={indexData}
													>
														{<Skeleton width="2rem" />}
													</td>
...
export const CountriesTable = memo(() => {
	...
	const { searchValue, setSearchValue, setOpenModal, searchRef, setCountryID } =
		useManageTable();
	const [activeSort, setActiveSort] = useState(1);
	const countriesQty = dataMap?.mapStats[mapTabSelected]?.data.rows;
	const columnTitle = dataMap?.mapStats[mapTabSelected]?.data.columns.map(
		(col) => col.name,
	);
	const [page, setPage] = useState(1);
	const tableKey =
		dataMap?.mapStats[mapTabSelected]?.data.columns[activeSort]?.name;
	const countryObject: Record<string, unknown>[] = countriesQty
		?.map((item) => {
		...
		.filter(
			(
				...
				return searchValue !== ""
					? String(Object.values(item)[0]?.displayText)
							.toLowerCase()
	...
	const elementsOnPageQtyTo =
		elementsOnPageQtyFrom - 1 + countriesQtyChunk[page - 1]?.length;
		...
		<>
			{!countriesQty ? (
				<p className="w-full text-center">
					Оформите подписку, чтобы увидкть больше информации
				</p>
			) : (
				<>
					{loading === true ? (
								...
								return (
									...
									>
										<div className="w-full ">
											<div className="overflow-x-auto">
												<div className="inline-block w-full align-middle rounded-xl overflow-hidden overflow-x-auto relative">
													<table className="min-w-[40rem] m_grey w-full ">
														<thead className="bg-dark_grey/50">
															<tr>
																{mapStat?.data.columns.map(
																	(headerTitle, indexHeaderTitle) => (
																		<th
																			scope="col"
																			...
																			key={indexHeaderTitle}
																			onClick={() =>
																				handleClickTableHeader(indexHeaderTitle)
																			}
																		>
																			<div className="flex items-center">
																				<span className="">
																						...
																						return itemObjVal ===
																							...
																							>
																								...
																								>
																									{itemObjVal.avatar ? (
																										<img
																											src={itemObjVal.avatar}
																											alt=""
																											className="h-6 w-6"
																										/>
																									) : null}
																									<span className="block text-white text-caption_r_desk">
																										{numbersFormatter(
																											itemObjVal.displayText,
																										)}
																									</span>
																								</div>
																							</td>
											...
											<div className="w-full flex justify-between items-center">
												<div className="items-center inline-flex space-x-2">
													...
													<p className="whitespace-nowrap px-5 py-3 text-xs text-medium_grey">
														Страница {page}
													</p>
													<button
														className="cursor-pointer"
												...
												<span className="whitespace-nowrap px-5 py-3  text-xs text-medium_grey">
													Страницы {elementsOnPageQtyFrom} -{" "}
													{elementsOnPageQtyTo} из {countriesQty?.length || "0"}
												</span>
											</div>
										</div>
									</div>
