...
const DeleteTrackLink = memo(() => {
	const { selectedTrack, setOpenPopUp, isRequestSent, setIsRequestSent } =
		useGetTracks();
			...
			{isRequestSent ? (
				...
				<div className="flex flex-col gap-5">
									...
									<div>
										...
										<p className="text-caption_s_desk text-medium_grey">
											{track?.artistLinks.map((artist, indexArtist) => (
												<span key={index}>
													{indexArtist !== track?.artistLinks - 1
														? `${artist.text}, `
														: artist.text}
												</span>
											))}
					...
					<p className="text-caption_r_desk text-medium_grey">
						Если ты считаешь, что данный трек или одна из ссылок на него
						добавлены ошибочно, ты можешь отправить нам запрос на удаление.
						Запрос будет рассмотрен в течение 3 дней.
					</p>
					<div className="flex flex-col gap-4">
						<h2 className="text-t2Regular text-light_grey">
							Что нужно удалить?<span className="text-secondary_red"> *</span>
						</h2>
						<div className="flex flex-col gap-3">
							<div className="flex gap-2 items-center">
								...
								<p className="text-caption_r_desk text-light_grey">
									Трек со всеми ссылками
								</p>
							</div>
							<div className="flex gap-2 flex-col">
								<div className="flex gap-2 items-center">
									<button
										onClick={() => setDeleteOption("deleteOnlyOneLink")}
										className="w-[18px] h-[18px] border border-solid border-secondary_dark_gray rounded-full flex justify-center items-center"
									>
										<div
								...
								{deleteOption === "deleteOnlyOneLink" && (
									<div>
										<label className="text-caption_m_desk text-medium_grey">
											Ссылка на трек{" "}
											<span className="text-secondary_red">*</span>
										</label>
										<input
											placeholder="https://"
