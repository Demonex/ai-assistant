...
const TableItemSocials = memo<{ track: any; social: any }>(
	({ social, track }) => {
		const filtered = track.links.filter((link) => {
			return !(
				link.source !== social.slug ||
				!social.tableLogo ||
				!social.tableLogo
			);
		});
		...
		return filtered.length > 1 ? (
			<div
				className="w-5 h-5 rounded-full flex justify-center items-center text-black group cursor-pointer relative"
				style={{
					backgroundColor: `${social.color}`,
				}}
			>
				{filtered.length}
				<div className="p-2 rounded-[4px] bg-medium_grey lg:bg-popup_gray hidden absolute group-hover:block top-full left-0 min-w-[420px]">
	...
	return (
		<div className="flex gap-0.5">
			{artistsList.map((artist, index) => (
				<p
					key={index}
					className="text-caption_s_desk text-medium_grey whitespace-nowrap"
				>
					{index !== artistsList.length - 1 ? `${artist},` : artist}
				</p>
			))}
		</div>
	);
...
const Catalogue = memo(() => {
	...
	const filteredTrackSocials = socialsWithAddedIcon.map((social) => {
		return clickedTrack?.links?.filter((link) => {
			return !(
				link.source !== social.slug ||
				!social.tableLogo ||
				!social.tableLogo
	...
	const source =
		socialsWithAddedIcon.length >= filteredTrackSocials?.length
			? socialsWithAddedIcon
			: filteredTrackSocials;
	const opposite =
		socialsWithAddedIcon.length >= filteredTrackSocials?.length
			? filteredTrackSocials
			: socialsWithAddedIcon;
	...
	return (
		<>
			<main className="h-full relative z-20 pt-4 xl:max-w-none w-full px-4 md:px-8 lg:pl-6 lg:pr-2">
				<header
					id="header"
					className="mb-4 md:flex justify-between items-center"
				>
					{isMobile || isTablet ? (
						<ArtistMobileHeader />
					) : (
				...
				<section className="w-full flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
					<div className="w-full flex gap-4 items-center ">
						...
						<div className="relative group hidden md:block">
							...
							<div className="p-2 rounded-[4px] flex gap-3 bg-popup_gray text-caption_r_desk absolute whitespace-nowrap bottom-full mb-1 opacity-0 group-hover:opacity-100 transition duration-300">
									...
									<th className="p-4 pl-[66px]">
										<div className="flex items-center gap-2">
											...
											<button onClick={() => setSortTracks(!sortTracks)}>
												<ArrowDropdown
													className={`fill-white ${sortDate ? "rotate-180" : ""}`}
												/>
											</button>
										</div>
									</th>
								...
								{tracks
									...
									.map((track, index) => {
										return (
											<tr
												key={index}
												className="border-b border-secondary_dark_gray"
											>
												<td className="flex items-center gap-6 p-4 max-w-[430px]">
													...
													<Link
														...
														onClick={() =>
															setSource(sample(track.links).source)
														}
													>
												...
												<ShowOnTabletAndDesktop>
													<td className="p-4 whitespace-nowrap text-caption_r_desk text-medium_grey">
														{DateTime.fromISO(track.createdAt).toFormat(
															"dd/MM/yyyy",
														)}
													</td>
												</ShowOnTabletAndDesktop>
							...
							<Dialog.Panel className="bg-popup_gray w-full h-screen p-4 md:p-8 overflow-auto">
								...
								<div className="flex flex-col gap-6">
									<div className="flex items-center gap-6">
										<img
											src={clickedTrack?.imageUrl}
											className="w-[6.25rem] h-[6.25rem] md:w-[7.5rem] md:h-[7.5rem] rounded-full"
										/>
										<div className="flex flex-col gap-2">
											<h1 className={"text-t1Mobile md:text-t1Regular  "}>
											...
											<p className="text-t2Regular text-medium_grey">
												{clickedTrack?.artistLinks?.map((item, i) => {
													return (
														<span key={i}>
															{i !== clickedTrack.artistLinks.length - 1
																? `${item.text},`
																: item.text}
														</span>
													);
									...
									<div className="hidden md:block">
										<div className="flex md:px-4 md:py-5 md:bg-[#48484840] rounded-t-xl md:gap-6 justify-between ">
											{socialsWithAddedIcon.map((social, indexSocial) => (
												<div key={indexSocial}>
										...
										<div className="md:p-4 flex md:gap-6 justify-between border-b border-secondary_dark_gray ">
											{socialsWithAddedIcon.map((social, indexSocial) => (
												<div key={indexSocial}>
										...
										<p className="text-[10px] font-normal leading-3 text-left text-medium_grey mt-4 hidden md:block">
											*компания Meta Platforms Inc., владеющая Facebook и
											Instagram, внесена в реестр экстремистских организаций, ее
											деятельность в России по поддержанию указанных соцсетей
											признана экстремистской деятельностью
										</p>
									</div>
									<div className="md:hidden">
										<div>
											{res.map((item, index) => {
												return (
													<div
														className="p-4 border-b border-secondary_dark_gray flex justify-between"
														key={index}
													>
														<div className="w-1/2  flex justify-center">
