...
export const ProfileShearingPage = memo(() => {
	...
	const outerLinksTrack = trackData?.links.map((source) => {
		const result = navbar[0].content[0].options.filter(
			(item) => item.slug === source.source,
		);
		return {
			url: source?.url,
			icon: result[0]?.logo,
				...
				<div className="pt-10 pb-5 flex flex-col gap-6 max-w-[35rem] px-4 md:px-8 lg:mx-[12.5rem] h-screen">
					<div className=" flex flex-col gap-4">
						...
						<div className="py-2 flex flex-col md:flex-row gap-6 lg:items-center">
							<img
								src={
									isArtist
										? artistData?.account?.imageUrl
										: trackData?.trackInfo?.avatar
								}
								alt=""
					...
					<div className="h-full overflow-y-auto">
						<div className="pb-2 border-b border-secondary_dark_gray">
							<p className="uppercase text-caption_s_desk text-medium_grey">
								ЭТОТ {isArtist ? "АРТИСТ" : "ТРЕК"} НА МУЗЫКАЛЬНЫХ ПЛАТФОРМАХ
							</p>
						</div>
						<ul className="h-full ">
							{isArtist
								? outerLinksArtist?.map(
										(link, index) =>
											link?.name && (
												<a
													key={index}
													...
													rel="noreferrer"
												>
													<li className="p-4 flex justify-between items-center border-b border-secondary_dark_gray/50 cursor-pointer">
					...
					<div className="-mt-2 flex flex-col gap-4">
						<p className="text-medium_grey text-[10px] ">
						...
						<div className="flex justify-between items-center">
							...
							<p className="text-caption_s_desk">
								Получи аналитику своих треков
							</p>
						</div>
					</div>
				</div>
