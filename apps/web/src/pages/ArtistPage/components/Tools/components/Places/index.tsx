	...
	cities: [
		{
			...
			artists: [
				...
				},
				{
					name: "OOMPH!",
					imageUrl:
						"https://i.scdn.co/image/ab6761610000e5ebc60b5748dbea18cf4311e910",
				},
				{
					name: "Lindemann",
					imageUrl:
						"https://i.scdn.co/image/ab6761610000e5eb77470ac1f20df10ee2fa9a07",
				},
				{
					name: "Eisbrecher",
					imageUrl:
				...
				},
				{
					name: "Static-X",
					imageUrl:
						"https://i.scdn.co/image/ab6761610000e5eb7f4a72d9541571d430c157c7",
				},
				{
					name: "Iron Maiden",
					imageUrl:
				...
				},
				{
					name: "Slipknot",
					imageUrl:
						"https://i.scdn.co/image/ab6761610000e5ebd0cdb283a7384a0edb665182",
				},
			],
...
const Places = memo(() => {
	...
	const [
		{ data: itemData, loading: itemDataLoading, error: itemDataError },
		itemDataFetch,
	] = useLazyFetch({
	...
	const [
		{ data: cityVenueData, loading: cityVenueLoading, error: cityVenueError },
		cityVenueFetchData,
	] = useLazyFetch({
		url: `${BACKEND_URL}/proxy/api/v1/marketing/related_artist_venue_cities`,
		cache: import.meta.env.VITE_CACHE === "true",
	});
	const generalSearchData = shuffle([
		...(searchData?.results[0]?.data || []),
		...(searchData?.results[1]?.data || []),
	]);
			...
			<section>
				<div className="py-4">
					<p className="text-caption_r_desk text-medium_grey">
						В этом разделе ты можешь узнать, в каких городах выступают похожие
						исполнители.
					</p>
				</div>
							...
							{searchData && value && (
								...
								>
									{generalSearchData.length === 0 && value && (
										<p className="text-t2Regular text-medium_grey px-5">
									...
									<div className="flex gap-2 px-5 mb-4">
										{searchData?.results.map((item, index) => (
											...
											>
												<p className="text-caption_m_desk capitalize">
													{item.type}
												</p>
											</div>
										))}
									...
									{tabIndex === undefined
										? generalSearchData.map((item, index) => (
												<div
													className={
														"py-3 px-5 border-b border-secondary_dark_gray "
													}
													key={index}
												>
													<Link
                                    ...
                                    idUnique: id
													...
													>
														<div className="flex items-center gap-4">
															...
															/>
															<div className="">
																<h3 className="text-t2Regular truncate">
																	{item.name}
																</h3>
																<p className="text-caption_s_desk text-medium_grey capitalize">
																	{item.type}
																</p>
															</div>
														</div>
										...
										: Object.entries(searchData.results[tabIndex] ?? {}).map(
												([, data], index) => {
													return (
							...
							<div>
								<div className="flex flex-wrap max-w-[37rem] gap-y-4 gap-x-6">
									{data.cities
										...
										.map((city, index) => (
											<div
												className="py-3 px-4 bg-[#27272780] rounded-xl flex justify-between w-fit gap-12"
												key={index}
											>
												<div className="flex gap-2 items-center">
													<GlobeIcon className="fill-yellow w-11" />
													<p className="text-t2Regular">
														{city.name.split(",").shift()}
													</p>
												</div>
												<div className="">
													<p className="text-caption_r_desk text-medium_grey text-right">
														{city.artists.length}+
													</p>
													<p className="text-caption_r_desk text-medium_grey">
														событий
