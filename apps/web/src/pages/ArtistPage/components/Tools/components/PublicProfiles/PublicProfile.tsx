	...
	summaryStats: [
		{
			...
			tooltip:
				"The number of times your Songshare Link pages have been visited.",
		},
		{
			titleText: "Clicks",
			subtitleText: "Track Shares",
						...
						cells: [
							{
								...
								avatar:
									"https://i.scdn.co/image/ab67616d00001e021108fef87966d83948b6a037",
								order: "zick zack",
								trackId: 3898517,
								idUnique: "tmcz2fha",
								hasInternalLink: true,
						...
						cells: [
							{
								...
								avatar:
									"https://i.scdn.co/image/ab67616d00001e0202add2c77fb6999e311a3248",
								order: "deutschland",
								trackId: 707656,
								idUnique: "mgaco9t2",
								hasInternalLink: true,
						...
						cells: [
							{
								...
								avatar:
									"https://i.scdn.co/image/ab67616d00001e028b2c42026277efc3e058855b",
								order: "sonne",
								trackId: 497989,
								idUnique: "kvqtrzbx",
								hasInternalLink: true,
						...
						cells: [
							{
								...
								avatar:
									"https://i.scdn.co/image/ab67616d00001e02cbe7573e1175f842e24b34c2",
								order: "amerika",
								trackId: 1312329,
								idUnique: "m79q0yjn",
								hasInternalLink: true,
						...
						cells: [
							{
								...
								avatar:
									"https://i1.sndcdn.com/artworks-slyngskUOu3Nr0nx-yoX8GQ-t300x300.jpg",
						...
						cells: [
							{
								...
								avatar:
									"https://i.scdn.co/image/ab67616d00001e0219e289286920bf38b0b05a78",
						...
						cells: [
							{
								...
								avatar:
									"https://i.scdn.co/image/ab67616d00001e02c1207c7f4f9fedffb38a3d3e",
				...
				>
					{index === item.cells.length - 1 ? (
						<div className="flex justify-center items-center">
							<ShareIconAccount className="fill-white" />
						</div>
					) : (
						<div className="flex gap-6 lg:gap-11 items-center ">
							...
							<div>
								<p
									className={`text-t2Regular  ${index === 0 ? "text-white" : "text-light_grey"}`}
								>
									{cell.displayText}
								</p>
								<div className="flex flex-wrap">
									{cell.artistLinks?.map((artist: any, index: number) => (
										<p
											className="text-caption_s_desk text-medium_grey"
											key={index}
										>
											{index !== cell.artistLinks.length - 1
												? `${artist.text},`
												: artist.text}
										</p>
									))}
...
const PublicProfile = memo(() => {
	...
	const dataWithIconTracks = data.chartData.trackChartData.data.map(
		(item, index) => {
			const findLogo = navbar[0].content[0].options.filter(
				(icon) => icon.slug === item.source,
			);
	...
	const dataWithIconArtist = data.chartData.entityChartData.data.map(
		(item, index) => {
			const findLogo = navbar[0].content[0].options.filter(
				(icon) => icon.slug === item.source,
			);
	...
	const tableRows = data.tableData[0].data.rows
		.sort((a, b) => {
			return String(b.cells[activeSort].order).localeCompare(
				String(a.cells[activeSort].order),
				undefined,
				{ numeric: true },
			);
		})
		.filter((item) => {
			if (search !== "") {
				return String(item.cells[0].order)
					.toLowerCase()
	...
	return (
		<>
			<ShowOnLaptopToDesktop>
				<div className="py-6 ">
					<p className=" text-caption_r_desk text-medium_grey">
						В этом разделе ты можешь посмотреть статистику по своему{" "}
						<span className="text-white">профилю</span>
					</p>
				</div>
			</ShowOnLaptopToDesktop>
			...
			<div className="w-full flex flex-col gap-4 lg:gap-6 mb-10">
				<TrackInfoModal
					openTrackInfoModal={openTrackInfoModal}
						...
						<div className="w-full flex flex-col lg:flex-row lg:justify-between lg:items-center gap-2">
							...
							<div className="flex gap-2">
								<button onClick={() => setDataType("tracks")}>
									<p
										className={`text-caption_m_desk py-2 lg:p-2 ${dataType === "tracks" ? "text-medium_grey" : "text-white"}`}
									>
										{data.chartData.trackChartData.subtitleText}
									</p>
								</button>
								<button onClick={() => setDataType("artist")}>
									<p
										className={`text-caption_m_desk py-2 lg:p-2 ${dataType === "artist" ? "text-medium_grey" : "text-white"}`}
									>
										{data.chartData.trackChartData.subtitleText}
									</p>
								</button>
									...
									<defs>
										<linearGradient id="Color0" x1="0" y1="0" x2="0" y2="1">
											<stop
												...
												stopColor={
													dataType === "tracks"
														? data.chartData.trackChartData.color
														: data.chartData.entityChartData.color
												}
												stopOpacity={1}
											/>
											<stop
              ...
              dataWithIconTracks.map((item, index) => {
					...
					<ShowOnMobileToTablet>
						<div className="w-full flex justify-between items-center  gap-4">
							<p className="text-caption_s_desk text-medium_grey ">
								*компания Meta Platforms Inc., владеющая Facebook и Instagram,
								внесена в реестр экстремистских организаций, ее деятельность в
								России по поддержанию указанных соцсетей признана экстремистской
								деятельностью
							</p>
							<InfoIcon className="fill-medium_grey min-w-5 h-5 " />
						</div>
					</ShowOnMobileToTablet>
					<ul className="xl:flex xl:flex-col gap-4 lg:gap-6 w-full xl:max-w-[20.5rem] grid grid-cols-1 md:grid-cols-2 mt-2 md:mt-[unset]">
				...
				<div className="w-full lg:bg-popup_gray/50 rounded-[20px] py-6 lg:p-8 flex flex-col gap-6">
					<div className="w-full flex flex-col md:flex-row justify-between md:items-center gap-6">
						...
						<div className=" w-full md:max-w-[320px] border border-dark_grey rounded-xl flex items-center gap-2 py-3.5 px-2.5 ">
										...
										{data.tableData[0].data.columns.map((title, index) => (
											...
											>
												<div className="flex gap-2 items-center">
													<p>{title.name}</p>
													{index !==
														data.tableData[0].data.columns.length - 1 && (
														<ArrowDropdown
															className={` ${activeSort === index ? "fill-white" : " fill-medium_grey"}`}
														/>
													)}
												</div>
											</th>
