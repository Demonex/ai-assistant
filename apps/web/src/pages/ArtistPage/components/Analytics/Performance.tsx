		...
		<SkeletonTheme baseColor="#C7D2FE0D" highlightColor="#C7D2FE12">
			<div className="p-4 w-full mb-5">
				<div className=" flex items-center gap-3 w-full justify-center pt-4">
					...
					<p className="text-sm lg:text-md text-transparent capitalize bg-gradient-to-r from-indigo-300 to-indigo-400 bg-clip-text">
						{<Skeleton width={100} />}
					</p>
				</div>
...
export const Performance = memo(() => {
	...
	const filteredData = chartData?.chart?.iconData?.filter(
		(item) => item.secondaryText === "total",
	);
			...
			{apiChartDataLoading === true ? (
				...
				<>
					<div className=" flex items-center w-full justify-between pb-4 border-b border-dark_grey mb-4">
						<p className="text-btnText capitalize text-light_grey">
							{t("performance")}
						</p>
					...
					{filteredData?.map((item, index) => {
						return (
							<div
								className="w-full overflow-y-scroll flex flex-col gap-3 "
								key={index}
							>
								<div className="text-white w-full">
									<div className="bg-transparent py-2 rounded-xl h-full flex justify-between items-center gap-10 ">
										<span className="text-medium_grey text-caption_r_desk">
											{t(item.text)}
										</span>
										<span className="text-t1Semi_mob md:text-t1Semi_deck">
											{item.count}
										</span>
									</div>
								</div>
