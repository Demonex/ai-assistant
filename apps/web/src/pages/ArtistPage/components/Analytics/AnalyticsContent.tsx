		...
		return (
			<Transition appear show={openTariffsModal} as={Fragment}>
				<Dialog
					...
					onClose={() => setOpenTariffsModal(false)}
				>
					<Transition.Child
						as={Fragment}
					...
					<div className="">
						...
						>
							<Dialog.Panel
								...
								style={{
									minWidth:
										isMobile || isTablet ? "unset" : `${maxWidthContainer}px`,
								}}
							>
								<div
									className="w-full flex justify-end"
									onClick={() => setOpenTariffsModal(false)}
								>
									<SecondaryCloseIcon className="stroke-white" />
								</div>
								<h1 className="py-2 text-t1Semi_deck text-light_grey mb-4">
									Оформи подписку и получи доступ ко всем функциям
								</h1>
								{/*<TariffsCards/>*/}
							</Dialog.Panel>
						</Transition.Child>
	...
	return (
		<>
			...
			<main className="h-full relative z-20 pt-4 xl:max-w-none w-full pl-6 pr-2 hidden lg:block">
				<header
					id="header"
					className="mb-6 md:flex justify-between items-center"
				>
					...
					<div className="h-full relative z-10">
						<div className="h-full flex overflow-y-auto flex-col">
							...
							<div className="flex gap-6 flex-col items-center h-fit ">
								<section
									className={"w-full flex gap-6 mt-6 flex-col xl:flex-row"}
								>
									<Chart
			...
			{openTariffsModal && (
				<TariffsModal
					openTariffsModal={openTariffsModal}
					setOpenTariffsModal={setOpenTariffsModal}
				/>
			)}
		</>
	);
