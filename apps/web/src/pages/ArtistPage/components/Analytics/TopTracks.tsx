			...
			<div className="w-full overflow-y-scroll flex flex-col gap-3 mt-2.5">
				{data.items[activeButton]?.length ? (
					data.items[activeButton]?.map((track, index) => {
						...
						return (
							<Link
								to={`/track/${track.idUnique}/${track.trackName?.split("/").pop()}?source=${source}`}
								...
								<div className="bg-transparent py-2.5 rounded-xl h-full flex justify-between items-center gap-5">
									<div className="flex gap-4 items-start">
										...
										<div className="flex flex-col gap-1.5 max-w-[15rem]">
											<p className="text-caption_r_desk whitespace-nowrap truncate">
												{track.primaryText}
											</p>
											<p className="text-caption_s_desk text-medium_grey capitalize line-clamp-1">
												{track.secondaryText}
											</p>
										</div>
									</div>
									<span className="text-caption_m_desk">
										{track.primaryValue}
									</span>
								</div>
							</Link>
...
export const TopTracks = memo(() => {
	...
	return (
		<div
			className={`w-full  flex flex-col lg:flex-row gap-4 lg:gap-6 ${trackData?.trackData?.listData?.length > 2 ? "flex-wrap" : ""} `}
		>
			{apiTrackDataLoading === true ? (
				...
				trackData?.trackData?.listData?.map((item, index) => {
					if (
						!(
							("buttons" in item && item.items.length) ||
					...
					return (
						<div
							className={
								"lg:bg-popup_gray/50 rounded-[20px] h-fit  lg:px-7 py-4 lg:py-8  w-full lg:max-w-[30rem]"
							}
							key={index}
						>
												...
												<div className="bg-transparent p-2 rounded-xl h-full flex justify-between items-center gap-10">
													<div className="flex gap-3 items-start">
														...
														<div className="flex flex-col gap-1.5 max-w-[9.125rem]">
															<span className="font-normal text-gray-300 text-caption_r_desk whitespace-nowrap truncate">
																{track.artistName}
															</span>
															{track?.trackName && (
																<p className="text-gray-400 font-light text-[10px] xl:text-[12px] capitalize">
																	{track.trackName}
																</p>
															)}
														</div>
													</div>
													<span className="text-t1Sem2_deck text-light_grey">
														{track.primaryValue}
													</span>
												</div>
											</Link>
