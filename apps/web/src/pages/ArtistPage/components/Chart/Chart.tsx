...
const TabsZoom = forwardRef<HTMLImageElement, TabsZoomProps>(
	({ zoom, setZoom, currentDataFiltered, refTabs }, refSettings) => {
		...
		useEffect(() => {
			...
			const buttons = Array.from(
				tabsParentRef.current.childNodes as NodeListOf<HTMLDivElement>,
			);
					...
					<div className="flex gap-10">
						{currentDataFiltered?.map((item, index) => (
							<div className="flex flex-col " key={index}>
								<h1
									className={`pl-1 text-caption_s_desk whitespace-nowrap uppercase text-medium_grey border-l-2 ${index === 0 ? "border-magenta" : "border-yellow"}`}
								>
									{t(item.text)}
								</h1>
...
const CustomTooltip = memo<CustomTooltipProps<number, string>>(
	({ active, payload }) => {
		...
		const formattedDate = DateTime.fromJSDate(date)
			.setLocale("ru")
			.toFormat("ccc d/MM/yyyy");
		return (
			<div className="flex flex-col bg-popup_gray text-white p-2 rounded-md gap-2">
				<p className="text-caption_s_desk text-medium_grey pb-1 border-b border-medium_grey capitalize">
					{formattedDate}
				</p>
				{payload.map(({ name, value }, index) => (
					<div className="flex gap-1" key={index}>
						<p
							className={`text-caption_s_desk uppercase text-medium_grey border-l-2 pl-1 ${index === 0 ? "border-magenta" : "border-yellow"}`}
						>
							{name}:{" "}
						</p>
...
export const Chart = forwardRef<any, any>(
	({ setPopupChart, popupChart }, ref) => {
		const { t } = useTranslation();
		...
		const currentDataFiltered = chartData?.chart?.iconData?.filter(
			(item, index) =>
				...
				? seriesData
						...
						.reduce((prev, current, index, array) => {
							...
							return index === array.length - 1
								? Object.entries<any>(prev).reduce<any>(
										(prev, [time, object = {}]) => {
			...
			switch (zoom) {
				...
				default: {
					return DateTime.fromMillis(
						get(chartDataGraph?.at(0), "time", Date.now()),
					).toMillis();
				}
			}
		}, [zoom, chartDataGraph]);
		...
		const endDate = useMemo(() => {
			return DateTime.fromMillis(
				get(chartDataGraph?.at?.(-1), "time", Date.now()),
			).toMillis();
		}, [chartDataGraph]);
		...
		const getTicks = useCallback(
			(_startDate: number, _endDate: number, ticks: number) => {
		...
		const fillTicksData = useCallback(
			(ticks: number[], data: DataType = []) => {
