		...
		{Array.from({ length: 10 }).map((_, index) => (
			...
			>
				<div className="mb-6 col-span-2 xl:mb-0 w-full xl:w-[70%] 2lg:w-[80%] relative">
					<div className="flex items-center justify-center rounded-md  p-[0.060rem] text-center bg-gray-900 aspect-square absolute w-[2rem] h-[2rem] top-[50%] -translate-y-1/2 -left-[2.75rem]">
						<Skeleton width={32} height={32} />
					</div>
					<div className="ring-1 ring-inset ring-white/5 text-white shadow-2xl rounded-xl w-full">
						<div className="bg-transparent p-4 rounded-xl h-full flex flex-col gap-3">
							<div className="flex items-center gap-3">
								...
								<div className="flex flex-col">
									...
									<p className="text-md">{<Skeleton width={150} />}</p>
								</div>
							</div>
							<div>
...
export const FeedContent = memo(() => {
	...
	const [{ data: feedDataFree, loading: loadingFree }, fetchFeedFree] =
		useLazyFetch({
			url: `${BACKEND_URL}/proxy/api/v1/activities/index`,
		});

	const [{ data: feedDataPaid, loading: loadingPaid }, fetchFeedPaid] =
		useLazyFetch({
			url: `${BACKEND_URL}/proxy-paid/v1/collaborators/activities`,
		});

	// eslint-disable-next-line @typescript-eslint/no-unused-vars
	const { subscriptions, /*subscribe, unsubscribe, */ isSubscribed } =
		useSubscriptions();
	...
	const feedData = useMemo(() => {
		return isSubscribed
			? {
					tracks:
						feedDataPaid?.data?.map(({ track_info, ...rest }) => ({
							...rest,
							...track_info,
						})) || [],
				}
	...
	useEffect(() => {
		...
		void fetchFeed({
			params: {
				[isSubscribed ? "songstats_collaborator_id" : "idUnique"]: id,
				[isSubscribed ? "limit" : "page"]: isSubscribed ? 50 : page,
				source,
			},
		});
					...
					<section className=" relative">
						<div className="relative z-10">
							<div className=" flex overflow-y-auto ">
								<div className="flex min-w-full items-center gap-4 justify-between pb-3 md:pb-4 lg:pb-[unset] lg:px-[unset] border-b border-[#33333380] lg:border-none pt-3 md:pt-[unset]">
										...
										<div className=" flex flex-col justify-center items-center gap-4">
											<div className="flex items-center gap-4">
												...
												<h1 className="text-t1Mobile md:text-t1Regular text-light_grey">
													Активность за последние 30 дней недоступна
												</h1>
											</div>
											{!profile ? (
												...
												!isSubscribed && (
													<p className="text-t2Regular text-medium_grey text-center">
														Подпишись на артиста, чтобы пользоваться всеми
														возможностями ленты и получать уведомления о новых
														действиях!
													</p>
												)
											)}
						...
						<div>
							<p className="text-medium_grey text-caption_r_desk">
								В прошлом месяце
							</p>
						</div>
						{source === "overview" ? (
							<ol
								className="relative py-4  grid xl:grid-cols-2 gap-4 lg:gap-6 lg:px-[unset]"
							...
							>
								{loading === true ? (
									...
									feedData?.tracks?.map((item, index) => {
										const getSourceActivity = navbar[0].content[0].options.find(
											(elem) => elem.slug === item.source,
										);
                           ...
                           const mm = new Date(date).toLocaleString('default', {month: 'short'});*/
												...
												<div className="p-4 bg-popup_gray/50 rounded-xl">
													<div className="flex gap-4">
														...
														<div className="w-full flex flex-col">
															<div className="flex w-full pb-2 border-b border-dark_grey gap-2 md:gap-4 md:items-center flex-col md:flex-row">
																<div className="flex gap-4">
																	{isMobile && (
																		<div className="w-7 h-7">
																			<img src={getSourceActivity?.logo} />
																	...
																	<img src={playlistedIcon} alt="" />
																</div>
																<div className="flex flex-col-reverse md:flex-row gap-2 md:gap-4 overflow-hidden">
																	<p className="text-t2Regular truncate max-w-[250px]">
																		{item.actionSubjectText || item.title}
																	</p>
																	<h4 className="text-caption_r_desk text-medium_grey whitespace-nowrap overflow-ellipsis overflow-hidden">
																		{item.actionDescriptorText ||
																			item.activity_text}
																	</h4>
																</div>
															</div>
																...
																<div className="items-center flex gap-4 justify-between">
																	<div className="items-center flex gap-4 ">
																		...
																		<div className="max-w-[180px] 2xl:max-w-full">
																			<div className="text-t2Regular truncate">
																				{item.trackName}
																			</div>
																			<div className="text-caption_s_desk text-medium_grey">
																				{item.artistName}
																			</div>
																		</div>
																	</div>
																	...
																	<a
																		href={item.externalUrl || item.activity_url}
																		target="_blank"
							...
							<div className="w-full">
								<h2 className="text-center text-t1Semi_deck">
									Subscribe to get full info
								</h2>
							</div>
						)}
					</section>
