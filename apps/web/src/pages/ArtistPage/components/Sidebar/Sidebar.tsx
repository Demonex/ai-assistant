	...
	return (
		<>
			{navigations.map((nav, index) => (
				...
				>
					<Link
						to={`/artist/${id}/${name}${nav.component ? `/${nav.component}` : ""}`}
...
const SkeletonSideBar = memo(() => (
	<SkeletonTheme baseColor="#C7D2FE0D" highlightColor="#C7D2FE12">
		<div className="flex flex-col items-center gap-6">
			...
			<div className="flex justify-center flex-wrap lg:text-center gap-2 list-none ">
...
export const SidebarMobile = memo(() => {
	...
	return (
		<div className="mb-[4.5rem] overflow-y-auto mt-[4.375rem] py-6 px-4 relative">
			<button
				className="absolute left-2 p-2 top-2 z-20"
			...
			<nav id="nav" className=" relative flex flex-col h-full">
				<div className="relative">
					{artistProfileLoading === true ? (
						...
						<div className="flex flex-col items-center gap-4 ">
							...
							<div className="flex flex-col items-center gap-4">
								<h1 className="text-h4Desctop">
									{artistProfile?.account?.name}
								</h1>
								<span className="text-caption_r_desk text-medium_grey uppercase">
									{artistProfile?.account?.country}
								</span>
							</div>
							...
							<div className={isSubscribed ? "" : "mt-2.5"}>
								...
								/>
								<div
									className={`w-full flex justify-center ${isSubscribed ? "" : "mt-4"}`}
								>
									<Link
										to=""
										className=" text-caption_m_desk text-light_grey text-center"
									>
										Как работает сервис?
									</Link>
								</div>
				...
				{
					<>
						<div className="flex justify-center flex-wrap gap-[18px] list-none mt-7">
							{socialsFiltered?.map((option, index) => (
								<Link key={index} to={option.link} target="_blank">
									<div className="bg-popup_gray p-1.5 rounded-full">
										<img src={option.logo} alt="" className="w-3.5 h-3.5" />
									</div>
								</Link>
							))}
						</div>
						<p className="text-[10px] text-medium_grey text-center mt-2 leading-3">
...
export const Sidebar = memo(() => {
	...
	return (
		<>
			<div className="hidden lg:block fixed z-20 inset-0  right-auto w-[15.25rem] mb-10 px-6 overflow-y-auto mt-[63px] md:mt-[103px] pt-6 border-r border-popup_gray">
				<nav
					id="nav"
					className="lg:text-sm lg:leading-6 relative flex flex-col h-full items-start"
				>
									...
									<div className={isSubscribed ? "" : "mt-2.5"}>
										{!isSubscribed && (
											<PrimaryButton
												...
												icon={subscribedIcon}
												onClick={() =>
													isSubscribed ? () => {} : setOpenModal(true)
												}
											/>
										)}
										<div
					...
					{
						<>
							<div className="flex justify-center flex-wrap lg:text-center gap-4 list-none py-4 mt-7">
								{socialsFiltered?.map((option, index) => (
									<Link key={index} to={option.link} target="_blank">
										<div className="bg-popup_gray p-1.5 rounded-full">
											<img src={option.logo} alt="" className="w-3.5 h-3.5" />
							...
							<p className="text-[10px] text-medium_grey text-start mt-2 leading-3">
								*компания Meta Platforms Inc., владеющая Facebook и Instagram,
								внесена в реестр экстремистских организаций, ее деятельность в
								России по поддержанию указанных соцсетей признана экстремистской
								деятельностью
