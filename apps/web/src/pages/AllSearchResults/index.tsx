	...
	return (
		<>
			{searchData.map((items, indexWrapper) => (
				<>
					{items?.title === "songs" ? null : (
						<div
							key={indexWrapper}
							className="py-3 px-4 bg-popup_gray/50 rounded-[20px] "
						>
							<h2 className="text-h4Desctop capitalize mb-8">
								{items.title} &nbsp;
								<span className="text-t1Regular text-medium_grey">
									({items.items.length})
								</span>
							</h2>
							<Swiper
									...
									<SwiperSlide key={indexSlide}>
										<div className="w-full flex justify-center">
											<div className="flex flex-col items-center gap-2.5">
												<div className="flex justify-center h-[7.5rem] w-[7.5rem] flex-none rounded-full p-1 border border-yellow">
													<div
...
export const AllSearchResults = memo(() => {
	const { searchData, searchValue } = useSearchData();
	const { navigation } = useParams<{
		navigation: string;
	}>();
	const { isMobile } = useSizes();
	const [showAllResults, setShowAllResults] = useState<number>(-1);
	const numberOfAllResults = searchData?.reduce(
		(acc, elem) => acc + elem.items.length,
		0,
	);
	return (
		<div className="w-full flex flex-col h-[100vh] items-center ">
			<Header />
			<div className="flex-1 w-full mt-[63px] md:mt-[103px] py-5 md:py-10 lg:py-[3.25rem] px-4 md:px-8 max-w-[1108px]">
				<div className="w-full flex flex-col gap-4 md:gap-6 lg:gap-10">
					<div className="w-full flex flex-col gap-4 md:gap-6">
						...
						<div className="flex w-full flex-col md:flex-row gap-2.5 md:justify-between md:items-center">
							<p className="text-t1Regular text-medium_grey">
								Результаты поиска: {numberOfAllResults}{" "}
							</p>
							<PrimaryButton
					...
					<div className="w-full flex flex-col gap-4 md:gap-6">
						...
						{searchData
							...
							.map((items, indexObject) => (
								<div
									className={
										"py-6 px-8 lg:px-9 rounded-[1.25rem] bg-popup_gray/50 w-full flex flex-col gap-6  "
									}
									key={indexObject}
								>
									...
									>
										{items?.items
													...
													{items?.title === "songs" ? (
														...
														>
															...
															>
																...
																<p className="text-t2Regular truncate max-w-[150px]">
																	{item?.title}
																</p>
															</div>
														</Link>
											...
											.filter((_, indexFilter) => {
												return showAllResults === indexObject
													? indexFilter < items.items.length
													: items.title === "songs"
														? indexFilter < 10
														: indexFilter < 5;
											})}
							...
							.filter((elem, wrapperIndexFilter) =>
								isMobile
									? wrapperIndexFilter === searchData.length - 1
									: { ...elem },
							)}
					</div>
				</div>
