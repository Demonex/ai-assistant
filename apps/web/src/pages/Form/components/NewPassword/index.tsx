...
export const NewPassword = memo(() => {
	...
	const [resetPassword, { isLoading }] = useMutation(
		`${BACKEND_URL}/auth/email/reset-password`,
		{
			onSuccess: () => setPasswordChanged(true),
			onError: () =>
				setError("password", { message: "Не удалось поменять пароль" }),
		},
	);
	...
	return (
		<div className="p-0 md:p-10 shadow rounded-2xl  md:bg-popup_gray w-full flex flex-col gap-4 md:gap-5 mb-[7.5rem] lg:mb-[unset]">
			{passwordChanged ? (
				<div className="flex flex-col gap-5">
					<div className="flex flex-col gap-2">
						...
						<p className="text-t2Regular lg:text-t2Regular text-medium_grey">
							Ты снова можешь пользоваться всеми преимуществами личного
							кабинета.
						</p>
					</div>
					<a href="/auth/sign-in">
						<button className="flex w-full justify-center rounded-xl bg-primary_blue px-3 py-3.5 text-caption_m_desk text-white hover:scale-105 transition duration-300">
							Войти
						</button>
					</a>
				</div>
				...
				<>
					<h1 className="text-t1Semi_mob lg:text-t1Semi_deck">
					...
					>
						<div>
							...
							<div className="mt-1.5">
								<div
									className={
										"flex w-full rounded-xl border border-dark_grey p-[.875rem]  focus:ring-0 focus:border-medium_grey"
									}
								>
									<input
							...
							<div className="mt-1.5">
								...
								>
									<input
										...
										{...register("password", {
											...
											validate: (value) =>
												value === comparePassword || "Пароли не совпадают",
											pattern: {
												value: /^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{8,}$/,
												message:
													"Пароль должен содержать не менее 8 символов (латинские буквы и цифры)",
											},
										})}
										onChange={({ target: { value } }) => {
								...
								{errors.password && (
									<p className="text-secondary_red text-caption_m_desk mt-1.5">
										{capitalize(String(errors.password.message))}
									</p>
								)}
							</div>
						</div>
						<div className="flex flex-col md:flex-row gap-3 md:gap-3.5 mt-2">
							<Link className="w-full" to="/auth/sign-in">
								<button className="flex w-full justify-center rounded-xl border border-solid border-medium_grey px-3 py-3.5 text-caption_m_desk text-white hover:scale-105 transition duration-300">
									Отмена
								</button>
							</Link>
							<button
