...
const DistributorsTitle = memo(() => {
	...
	return (
		...
		>
			...
			<p className="text-white text-t2Regular md:text-t1Regular ">
				Организуй концерты именно там!
			</p>
		</div>
	);
});
const FansTitle = memo(() => {
	...
	return (
		...
		>
			...
			<p className="text-white text-t2Regular md:text-t1Regular ">
				Поддерживай их на концертах именно там!
			</p>
		</div>
	);
});
		...
		>
			{customerName ? (
				...
				>
					...
					<p className="text-white text-t2Regular md:text-t1Regular ">
						Смотри, где есть твоя фан-база,
						{isTablet || isMobile ? null : <br />} и планируй там концерты
					</p>
				</div>
			)}
