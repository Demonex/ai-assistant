...
const Telegram = memo(() => {
	...
	return (
		...
		>
			...
			>
				<div className="py-6 px-5 md:py-[4.375rem] lg:py-[7rem] lg:px-[3.125rem] flex h-full flex-col items-center md:items-start gap-6 md:gap-10">
					<h1
						style={{
							fontSize:
								isTablet || isMobile ? `${h1SizeMobile}px` : `${h1Size}px`,
						}}
						className="text-h2Mobile md:text-h2Desctop text-center md:text-left md:w-2/3 lg:w-1/2"
					>
						Новые релизы сервиса и&nbsp;интересные{isTablet ? <br /> : null}{" "}
						новости в&nbsp;нашем телеграм-канале
					</h1>
					<PrimaryButton
						title="подписаться "
						isIcon={false}
