...
const Platforms = memo(() => {
	...
	const styles = useMemo(() => {
		return Array.from({ length: platformIcons.length })
			.map((_, i) => {
				const percent25 = [randomInt(10), randomInt(5)];
				const percent50 = [randomInt(20), randomInt(20)];
				const percent75 = [randomInt(10), randomInt(5)];
				return `
		...
		>
			...
			{customerName === "distributors" ? (
				<h1
					...
					style={{
						fontSize:
							isMobile || isTablet ? `${h1SizeMobile}px` : `${h1Size}px`,
					}}
				>
					Анализируй музыкальные релизы в&nbsp;одном
					<br /> сервисе и&nbsp;принимай обоснованные
					<br /> решения для развития
				</h1>
			) : (
				<h1
					className="text-h2Desctop font-bold lg:font-black"
				...
				>
					Аналитика твоей музыки <br /> с 14+ платформ в одном месте
				</h1>
			)}
			<div
				className="w-full relative"
				style={{ height: `${platformsContainerHeight}px` }}
			>
					...
					return (
						...
						>
							<img
								...
								key={index}
								className={
									"hover:scale-110 cursor-pointer transition duration-300 "
								}
							/>
						</Link>
					);
				...
				<div
					className=" items-center gap-11 absolute -translate-x-1/2 -translate-y-1/2 hidden lg:flex w-full"
					style={{
				...
				>
					...
					<p className="text-t1Regular">
						И еще 9 музыкальных
						<br /> платформ в приложении Rifify
					</p>
				</div>
			</div>
			<div
			...
			>
				...
				<p className="text-[1.2rem] md:text-t1Regular">
					И еще 9 музыкальных
					<br /> платформ в приложении Rifify
				</p>
			</div>
