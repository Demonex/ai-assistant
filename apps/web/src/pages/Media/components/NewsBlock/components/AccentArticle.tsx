...
const news = {
	...
	subtitle:
		"Режиссер Мишель Гондри рассказывает о том, почему видео получилось таким, что значат все эти персонажи, кто ставил танцы и рочие секреты съемочной площадки",
	...
	return (
		<div className="py-5">
			<div className="bg-popup_gray hover:bg-secondary_dark_gray rounded-2xl p-5 lg:p-8">
				<div className="w-full flex flex-col-reverse lg:flex-row lg:justify-between gap-10 lg:gap-[5.6rem] cursor-pointer">
							...
							<div className="flex flex-col gap-4">
								...
								<div className="flex flex-col gap-4">
									<h1
										...
										className="text-h2Mobile md:text-h2Medium lg:text-h2Desctop line-clamp-2 lg:line-clamp-3"
									>
										{post?.title}
									</h1>
									<p className="text-t2Regular line-clamp-2 lg:line-clamp-3">
										{post?.description}
									</p>
								</div>
							</div>
							<div className="">
								<span className="text-captionText text-medium_grey font-normal">
									{post?.createdAt &&
										DateTime.fromISO(post.createdAt).toFormat("d MMM yyyy")}
								</span>
							</div>
						</div>
					</Link>
					...
					>
						<div
							className="w-full h-full bg-cover bg-no-repeat bg-center rounded-[16px]"
							style={{
