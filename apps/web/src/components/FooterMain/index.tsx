					...
					<div>
						{platforms.map((platform, index) => (
							<div key={index} className="flex flex-col gap-5">
								<h2 className=" text-btnText text-medium_grey">
									{platform.title}
								</h2>
								{platform.list.map((item, i) => (
									<Link to={item.link} key={i} onClick={scrollToTop}>
										<p
											className={
												"capitalize text-t2Regular hover:text-medium_grey cursor-pointer"
											}
										>
											{" "}
											{item.name}
				...
				<div className="flex flex-col gap-5 py-5 md:py-[unset] border-b border-dark_grey md:border-none">
					...
					<div className="flex flex-col gap-5">
						{footerInfo.map((item, i) => (
							<p
								className={` text-t2Regular hover:text-medium_grey cursor-pointer ${i === footerInfo.length - 1 ? "underline underline-offset-4 decoration-[#125BFF] hover:text-primary_blue " : "hover:text-medium_grey "}`}
								key={i}
								onClick={() => {
									if (item.link) {
										navigate(item.link);
