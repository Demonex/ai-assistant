...
const MenuItemsWrapper = memo<MenuItemsWrapperProps>(({ children }) => {
	return (
		<div className="w-fit pt-[1.5rem]">
			<div className=" bg-[#272727] overflow-hidden px-8 py-6 min-w-[35.25rem]">
				{children}
			</div>
		</div>
	);
		...
		case navbar[0].title: {
			return (
				<MenuItemsWrapper>
					...
					<p className="text-captionText text-medium_grey text-xs max-w-[31.25rem] mt-2">
		...
		return (
			<a className="flex flex-row items-center cursor-pointer" href={link}>
				<p
					...
					className={
						"font-medium capitalize  whitespace-nowrap hover:text-medium_grey"
					}
				>
					{title}
				</p>
			...
			<Popover className="flex items-center relative">
				{({ open }) => (
					...
					>
						...
						>
							<Popover.Panel className="absolute z-10 w-screen max-w-sm px-4 mt-0 transform sm:px-0 lg:max-w-3xl ">
								<div
