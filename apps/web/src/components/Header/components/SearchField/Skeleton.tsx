...
export const SkeletonListItems = memo<SkeletonListItemsProps>(
	({ items = [] }) => {
		...
		return (
			<div aria-hidden="true" className="-m-[2px] opacity-25 leading-none">
				{(items as string[]).map((text, index) => (
					<span
						className="inline-block rounded-sm m-[2px] bg-grey-400"
						key={index}
					>
		...
		<>
				...
				<span className="block py-[2px] text-smaller">
					<div aria-hidden="true" className="-m-[2px] opacity-25 leading-none">
						<span className="inline-block rounded-sm m-[2px] bg-grey-400">
							<span className="text-transparent">best</span>
						</span>
						<span className="inline-block rounded-sm m-[2px] bg-grey-400">
			...
			<ul role="listbox" aria-labelledby="search-label" id="search-list">
				{Array.from({ length: items }).map((_, index) => (
					<li
						role="option"
						className="relative border-b border-[#e2e8f00d]"
						key={index}
					>
						<a
						...
						>
							<div className="flex items-start overflow-hidden">
								<div className="flex justify-center h-[24px] w-[16px] pb-[4px] mr-[16px] mt-[4px] flex-none transition-colors duration-75 ease-out text-grey-400">
									<div className="block h-[16px] w-[16px] bg-grey-400 opacity-25"></div>
								</div>
								<div className="flex flex-col truncate py-[4px]">
									<div className="flex items-center truncate text-sm leading-tight mt-[4px] xl:mt-[2px] mb-[2px] xl:order-2 text-grey-600">
							...
							<div
								hidden={true}
								className="w-[24px] ml-[8px] p-[2px] flex-none"
							></div>
						</a>
					</li>
				))}
