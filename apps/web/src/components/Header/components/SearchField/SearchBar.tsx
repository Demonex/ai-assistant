...
const SearchField = memo(() => {
	...
	const { isOpenSearchModal, setIsOpenSearchModal, isShowAll } =
		useOpenModalSearch();
	...
	useEffect(() => {
		...
		setSearchData(
			Object.entries(apiData.groupedResults)
				.sort()
				.map(([k, v]: [string, any[]], ig) => {
					return {
						title: k,
						items: v.map((item, ii) => ({
							title: item.name,
	...
	const isLoading = useMemo(
		() => Boolean(value && (loading || fakeLoading)),
		[value, loading, fakeLoading],
	);
	useEffect(() => {
			...
			<Transition appear show={isOpenSearchModal} as={Fragment}>
				...
				>
					...
					<div className="fixed inset-0 overflow-y-auto">
						<div className="flex flex-col min-h-full px-4 md:px-8 py-5 gap-5 md:gap-6 lg:px-[10.5rem] text-center laptop:pt-16">
							...
							>
								<Dialog.Panel className=" transition-all w-full flex gap-10 transform items-start justify-center ">
									<div
									...
									>
										...
										<div className="flex items-center px-3.5 md:px-6 py-3 bg-popup_gray md:bg-[unset] rounded-xl">
											<div className="w-full h-full">
												...
												>
													...
													<div className="relative flex w-full items-center md:border-r border-dark_grey">
														<input
															ref={inputRef}
															value={value}
															onFocus={onFocus}
														...
														>
															<SecondaryCloseIcon className="stroke-medium_grey hover:stroke-[white]" />
														</button>
													</div>
												</form>
											</div>
											<button
												onClick={() => setIsOpenSearchModal(false)}
												className=" hidden md:block h-full pl-8 pr-2 text-light_grey text-t2Regular hover:text-medium_grey transition-fast-out "
											>
												Отмена
											</button>
										</div>
											...
											<>
												...
												>
													...
													>
														<div
															className={
																isLoading ? "pointer-events-none" : "wrapper"
															}
														>
															{isLoading ? (
																<Skeleton />
																...
																<div className="px-5 pt-4 pb-20 mb:border-b border-b-secondary_dark_gray">
																	<p className="text-medium_grey text-t2Regular">
														...
														{isShowAll && (
															<Link
																...
																onClick={() => setIsOpenSearchModal(false)}
															>
																<div className="w-full px-1 py-5 md:p-5 sticky bottom-0 border-t border-secondary_dark_gray gap-4 group items-center cursor-pointer hidden md:flex">
																	<p className="text-btnText group-hover:text-medium_grey">
																		Смотреть все результаты
																	</p>
																	<ChevronRight className="w-5 h-5 fill-white group-hover:fill-medium_grey" />
																</div>
															</Link>
														)}
													</div>
													{searchData?.length === 0 ? null : (
														<div className="border-t border-l border-secondary_dark_gray hidden md:block w-[40%] flex-none overflow-y-auto ">
						...
						{searchData?.length === 0 || !isShowAll ? null : (
							<Link
								to={`/all_search_results?q=${value}`}
								onClick={() => setIsOpenSearchModal(false)}
							>
								<div className="w-full p-5 sticky bottom-0 border-t border-secondary_dark_gray justify-between items-center cursor-pointer flex md:hidden bg-[#0C0C0C]">
									<p className="text-btnText hover:text-medium_grey">
										Смотреть все результаты
									</p>
