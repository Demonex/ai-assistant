...
export const SearchResultsPanel = memo<SearchResultsPanelProps>(
	({ closeModal }) => {
       ...
       setData
		...
		const onHover = useCallback(
			(indexGroupSelected: number, indexItemSElected: number) => {
				setSearchData((searchData) =>
					searchData.map((group, indexGroup) => {
						group.items.forEach((item, indexItem) => {
							item.selected =
								indexItem === indexItemSElected &&
								indexGroup === indexGroupSelected;
						});
						return group;
					}),
				);
		...
		const artistSelected = useMemo<Artists[number] | undefined>(
			() =>
				searchData.reduce<any>((prev, group) => {
					return prev ? prev : group.items.find((item) => item.selected);
				}, undefined),
			[searchData],
		);

		const [prevIndexGroup, prevIndexItem] = useMemo<[number, number]>(
			() =>
				searchData.reduce<any>((prev, group, _indexGroup, array) => {
					...
					return [
						indexGroup,
						indexItem === -1 ? array[indexGroup].items.length - 1 : indexItem,
					];
				}, []),
			[searchData],
		);

		const [nextIndexGroup, nextIndexItem] = useMemo<[number, number]>(
			() =>
				searchData.reduce<any>((prev, group, _indexGroup, array) => {
					const isSelectedGroup = group.items.find((item) => item.selected);
					if (!isSelectedGroup) {
					...
					const indexGroup = array[_indexGroup].items[indexItem]
						? _indexGroup
						: _indexGroup + 1 === array.length
							? (() => {
									indexItem = 0;
									return 0;
								})()
		...
		const onEnter = useCallback(() => {}, [artistSelected]);
		...
		return (
			<>
				<div className="px-5 py-4 flex items-center justify-start gap-2 mt-2.5 md:mt-0 flex-wrap">
					{searchData?.map(({ title }, indexGroup) => {
						return (
							...
							>
								<h2
									className={`capitalize text-caption_m_desk  ${selectedTab === indexGroup ? "text-[black]" : " "}`}
								>
									{title}
								</h2>
							</button>
				...
				{Object.entries(searchData[selectedTab] ?? {}).map(
					([, value], index) => {
						return (
							<div key={index}>
								{Array.isArray(value) &&
									value
										...
										.map((item, indexItem) => (
											<div
												className="py-3 px-5 border-b border-secondary_dark_gray hover:bg-gradient-to-r from-[#3272FB] to-[#5A31FF] group"
												...
												>
													<div className="flex items-center gap-4">
														...
														<div className="">
															...
															<p className="text-caption_s_desk text-medium_grey capitalize  group-hover:text-light_grey">
																{item.secondaryText}
															</p>
														</div>
													</div>
