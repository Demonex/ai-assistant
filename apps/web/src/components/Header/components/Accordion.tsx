...
const AccordionItem = memo<AccordionItemProps>(
	({ title, items, index, activeIndex, setActiveIndex, link }) => {
		...
		return (
			<div>
				...
				>
					<p className=" text-h3Mobile md:text-h3Desctop ">{title}</p>
					{items.length ? (
						<div
							className={`plusminus_animation w-10 h-10  ${accordionOpen ? "active" : ""}`}
						/>
					) : (
						""
					)}
						...
						<div className="overflow-hidden">
							{items.map((itemContent, indexItem) => (
								<div key={indexItem}>
									<div
										key={indexItem}
										className="md:px-8 py-4 grid grid-cols-2 gap-4"
									>
										{itemContent.options.map((option, index) => (
											...
											>
												{option.logo ? (
													<div className="">
														<img src={option.logo} className="w-6 h-6" alt="" />
													</div>
												) : null}
												<Link
									...
									{index === 0 ? (
										<p className="text-captionText text-[0.813rem] text-medium_grey">
