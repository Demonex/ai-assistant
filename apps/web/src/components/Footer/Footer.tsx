...
export const Footer = memo(() => {
	...
	const MenuItemPlatformsOptions: [FooterNavBarItems, FooterNavBarItems] =
		useMemo<any>(() => {
			return chunk(
				get<any, any>(MenuItemPlatforms, "content", []).reduce(
					(prev, next) => [...prev, ...get(next, "options")],
					[],
				),
				10,
			);
	...
	const MenuItemUseCasesOptions: [FooterNavBarItems] = useMemo<any>(() => {
		return chunk(
			get<any, any>(MenuItemUseCases, "content", []).reduce(
				(prev, next) => [...prev, ...get(next, "options")],
				[],
			),
			10,
	...
	const MenuItemResourcesOptions: [FooterNavBarItems] = useMemo<any>(() => {
		return chunk(
			get<any, any>(MenuItemResources, "content", []).reduce(
				(prev, next) => [...prev, ...get(next, "options")],
				[],
			),
			10,
	...
	return (
		<footer className="relative border-t bg-gray-900 border-white/5 w-full">
			<svg
				className="absolute blur-3xl right-0 opacity-50"
				width="50%"
				height="100%"
				viewBox="0 0 400 400"
				fill="none"
				...
				<div className="xl:grid xl:grid-cols-3 xl:gap-8">
					<div className="text-white xl:col-span-1 w-full text-center laptop:w-auto laptop:text-start">
						<div className="flex flex-col items-start gap-6 w-full">
							<div className="flex flex-row items-center cursor-pointer gap-x-0.5 w-full justify-center laptop:w-auto laptop:justify-normal">
								<img src={logo} alt="logo" className="w-[30px] h-[30px]" />
								<h2 className="text-white font-bold text-[20px]">Rifify</h2>
							</div>
							<div className="flex flex-col gap-2 w-full justify-center laptop:w-auto laptop:justify-normal">
						...
						<div className="grid grid-cols-2 gap-8 mt-12 xl:mt-0 xl:col-span-2">
							<div className="md:grid md:grid-cols-2 md:gap-8">
								{MenuItemPlatformsOptions.map((items, index) => (
									<div key={index}>
										<h3
											className={`uppercase text-white ${index > 0 ? "opacity-0 select-none" : ""}`}
										>
											{MenuItemPlatforms.title}
										</h3>
											...
											<ul role="list" className="mt-4 space-y-2" key={index}>
												{
													<li className="flex items-center gap-x-4">
														...
														<a className="inline-flex items-center text-sm text-gray-300 hover:text-white cursor-pointer">
															{item.name}
														</a>
													</li>
												}
							...
							<div className="md:grid md:grid-cols-2 md:gap-8">
								{MenuItemUseCasesOptions.map((items, index) => (
									<div key={index}>
										<h3
											className={`uppercase text-white ${index > 0 ? "opacity-0 unselectable" : ""}`}
										>
											{MenuItemUseCases.title}
										</h3>
										{items.map((item, index) => (
											<ul role="list" className="mt-4 space-y-2" key={index}>
												{
													<li className="flex">
														<a className="inline-flex items-center text-sm text-gray-300 hover:text-white cursor-pointer capitalize">
															{item.name}
														</a>
													</li>
												}
			...
			<div className="px-5 py-12 mx-auto border-t border-gray-800 sm:px-6 md:flex md:items-center md:justify-between lg:px-20 backdrop-blur-3xl">
				<div className="flex justify-center mb-8 space-x-6 md:order-last md:mb-0">
					<span className="inline-flex justify-center w-full gap-4 lg:ml-auto md:justify-start md:w-auto">
						<a className="w-[16px] h-[16px] fill-gray-600 hover:fill-white transition fill 0.2s ease-in-out cursor-pointer">
