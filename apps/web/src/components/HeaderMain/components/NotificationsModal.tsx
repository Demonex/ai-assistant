...
const NotificationItemsAll = memo(({ items }: any) => {
	const sampleAll = Array.from({ length: 50 }).map(() =>
		sample(testNotifs[0].events),
	);
	return sampleAll.map((item, index) => (
		<div
			key={index}
			className="flex gap-2 items-center pb-4 border-b border-secondary_dark_gray"
		>
			...
			<p className="text-caption_r_desk">
				...
				<span className="text-medium_grey">
					{item.eventObject.subscribers}&nbsp;
				</span>
				...
				>
					<div className="fixed inset-0 w-screen overflow-y-auto ">
						<div className="w-full md:w-[unset] flex min-h-full items-center justify-center absolute right-0 top-0">
							<Dialog.Panel className=" bg-popup_gray w-full md:max-w-[33.7rem] ">
									...
									<div className="pt-2 pb-4 ">
										<ul className="flex items-center gap-2 justify-start overflow-y-scroll flex-wrap">
											{testNotifs.map((type, index) => (
												<li
													onClick={() =>
														setNotificationsTypeSelected(type.type)
													}
													key={index}
								...
								<ul className="py-4 flex flex-col gap-4 px-4 md:px-8">
									<div className="pt-4 mt-[100px]">
										<p className="py-2 text-caption_s_desk text-medium_grey">
											Сегодня
										</p>
									</div>
