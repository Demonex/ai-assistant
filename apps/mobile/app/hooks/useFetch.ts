...
const fetch = setupCache(Axios.create({
  ...
  storage: buildMemoryStorage(false, false, false),
  ttl: 1000 * 60 * 60 * 24
});
...
const lazyMemo = memoize((url: string, method?: AxiosRequestConfig["method"], cache?: boolean) => {
...
export const useLazyFetch = <T = any>({ url, method, cache }: {
  url?: string,
  method?: string,
  cache?: boolean
} = {}) => {
