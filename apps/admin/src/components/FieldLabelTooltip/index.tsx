...
import { useForm, useEditDepth, useTranslation, useLocale, Tooltip } from "@payloadcms/ui";
...
export const FieldLabelTooltip: (props: GenericLabelProps) => (React.JSX.Element | null) = (props) => {
  const {
    as: Element = "label",
    hideLocale = false,
        ...
        )}
        {
          Object.keys(tooltips).includes(schemaPath) && (
            <button
              ...
              type="button"
            >
              <Tooltip className={`${baseClass}__tooltip`} show={showTooltip} alignCaret="left">
                {tooltips[schemaPath]}
              </Tooltip>
              ?
            </button>
