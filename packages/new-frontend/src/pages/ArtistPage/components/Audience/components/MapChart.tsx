import {color, useTheme, percent, ease, Circle, create} from '@amcharts/amcharts4/core.js';
import {MapChart, projections, MapPolygonSeries, MapImageSeries} from '@amcharts/amcharts4/maps.js';
...
export default memo(() => {
  ...
  const dataMap = useMemo<Record<string, unknown>[]>(() => {
    ...
    const result = data?.mapStats?.reduce<any>((result, {data: {columns, rows}}, i) => {
      ...
      return i + 1 === data?.mapStats.length ? Object.entries<Record<string, unknown>>(result).map(([id, values]) => {
        return {
          id,
          color: findClosest(colors, 'lng', rows.find(({countryCode}) => countryCode === id)?.lng)?.color || sample(colors)?.color,
          ...values
        };
      }) : result;
    }, {});
  ...
  const getZoomLevel = useCallback((mapPolygon: MapPolygon) => {
    ...
    return Math.min(mapChartRef.current.seriesWidth / (w * 2), mapChartRef.current.seriesHeight / (h * 2));
  }, []);
  ...
  // select a country
  const selectCountry = useCallback((mapPolygon: MapPolygon) => {
  ...
  const rollOverCountry = useCallback((mapPolygon: MapPolygon) => {
    if (mapType === MAP_TYPE.globe) {
      return;
    }
    resetHover();
    if (mapPolygon) {
      ...
      const image = bubbleSeriesRef.current.getImageById(mapPolygon.dataItem.id);
      if (image) {
  ...
  const rollOutCountry = useCallback((mapPolygon: MapPolygon) => {
    if (mapType === MAP_TYPE.globe) {
      return;
    }
    const image = bubbleSeriesRef.current.getImageById(mapPolygon.dataItem.id);
  ...
  useLayoutEffect(() => {
    ...
    mapChartRef.current.homeZoomLevel = !isMobile && !isTablet && !isLaptop ? 1.2 : 0.8;
    mapChartRef.current.homeGeoPoint = mapType === MAP_TYPE.globe ? {
      longitude: 10,
      latitude: 25
    } : {
      longitude: 2,
    ...
    mapChartRef.current.projection = mapType === MAP_TYPE.globe
      ? new projections.Orthographic
      : new projections.Miller();
    mapChartRef.current.panBehavior = mapType === MAP_TYPE.globe ? 'rotateLongLat' : 'move';

    // when map is globe, background is made visible
    mapChartRef.current.backgroundSeries.mapPolygons.template.polygon.fillOpacity = .05;
    mapChartRef.current.backgroundSeries.mapPolygons.template.polygon.fill = colors.background;
    ...
    polygonSeriesRef.current = mapChartRef.current.series.push(new MapPolygonSeries());
    ...
    // Bubble series
    bubbleSeriesRef.current = mapChartRef.current.series.push(new MapImageSeries());
    bubbleSeriesRef.current.data = [...dataMap];
    ...
    bubbleSeriesRef.current.tooltip.background.fillOpacity = .25;
    ...
    imageTemplate.events.on('over', ({target}) => {
      rollOverCountry(polygonSeriesRef.current.getPolygonById(target.dataItem.id));
    });
    imageTemplate.events.on('out', ({target}) => {
      rollOutCountry(polygonSeriesRef.current.getPolygonById(target.dataItem.id));
    });
    imageTemplate.events.on('hit', ({target}) => {
      selectCountry(polygonSeriesRef.current.getPolygonById(target.dataItem.id));
    });
    ...
    imageTemplate.adapter.add('latitude', (latitude, target) => {
      const polygon = polygonSeriesRef.current.getPolygonById(target.dataItem.id);
      if (polygon) {
