    ...
    <>
      {
        track.items.map((item, index) => (
          <tr className='flex justify-between items-center py-4 border-b border-secondary_dark_gray w-full' key={index}>
            <td className='w-[65%] flex gap-4'>
              <img src={item.imageUrl} alt='' className='w-11 h-11 rounded-full'/>
              <div>
                <p className='text-t2Regular'>{item.primaryText}</p>
                <p className='text-caption_s_desk text-medium_grey'>{item.artistName}</p>
              </div>
            </td>
            {
              item.values.map((val, indexVal) => (
                <td key={indexVal} className='flex-1 px-3 text-caption_r_desk text-light_grey'>{val}</td>
              ))
            }
          </tr>
        ))
        ...
        <div className="">
          ...
          >
            <Dialog.Panel
              ...
              style={{
                minWidth: isMobile || isTablet ? 'unset' : `${minWidthContainer}rem`
              }}>
              ...
              <div className='flex flex-col gap-6'>
                <div className='py-2 flex items-center gap-4'>
                  ...
                  <h1 className='text-t1Semi_deck'>{dataPopup?.popupInfo?.countryName}</h1>
                </div>
                <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-4 lg:gap-6'>
                  {
                    dataPopup?.popupInfo?.summaryData?.map((item, index) => (
                      <div key={index}
                        ...
                        <h2 className='text-caption_r_desk text-medium_grey'>{item.titleText}</h2>
                ...
                <div>
                  <h3 className='text-btnText text-light_grey mt-2 mb-4'>Статистика по трекам</h3>
                  <table className='w-full mb-[7.5rem] hidden md:table'>
                    ...
                    <div className='py-3 px-8 w-full bg-yellow flex items-center gap-.5'>
                      <p
                        className='text-caption_r_desk text-black'>Страна: <span>{dataPopup?.popupInfo?.countryName}</span>
                      </p>
                    </div>
                    <tbody>
                    {
                      dataPopup?.popupInfo?.listData.map((track, index) => (
                        <div key={index} className='px-8'>
                          <h1 className='py-4 text-t1Regular'>{track.header.primaryText}</h1>
                    ...
                    <div>
                      {
                        dataPopup?.popupInfo?.listData?.map((item, index) => (
                          <div className='mt-4' key={index}>
                            <h2 className='text-t1Regular'>{item?.header?.primaryText}</h2>
                            <ul>
                              {
                                item.items.map((track, trackIndex) => (
                                  <li key={trackIndex} className='py-4 flex flex-col gap-4 border-b border-secondary_dark_gray'>
                                    <div className='flex gap-4 items-center'>
                                      <img src={track.imageUrl} alt='' className='w-11 h-11 rounded-full'/>
                                      <div>
                                        <p className='text-t2Regular'>{track.primaryText}</p>
                                        <p className='text-caption_s_desk text-medium_grey'>{track.artistName}</p>
                                      </div>
                                    </div>
                                    <div>
                                      <div className='w-full grid grid-cols-2 gap-2'>
                                          {
                                            dataPopup?.popupInfo?.listDataHeaders.map((header, indexHeader) => (
                                              <p key={indexHeader} className='text-caption_r_desk text-light_grey'>{header}</p>
                                            ))
                                          }
                                          {
                                            track.values.map((val, indexVal) => (
                                              <p key={indexVal} className='text-caption_r_desk text-light_grey'>{val}</p>
                                            ))
                                          }
                                      </div>
                                    </div>
