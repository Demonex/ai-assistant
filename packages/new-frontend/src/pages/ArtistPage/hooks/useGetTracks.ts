...
const _useGetTracks = () => {
  ...
  useEffect(() => {
    const result = data?.tracks?.filter((item) => {
      ...
      const found = item?.trackName.toLowerCase().includes(search.toLowerCase().trim())
      return found
    });
    setTracks(result)
  }, [search, data]);
  ...
  useEffect(() => {
    setTracks(() => {
      if (!sortDate) {
        return tracks?.sort((a, b) => dateToNum(b.createdAt) - dateToNum(a.createdAt))
      }
...
export const useGetTracks = () => useSharedHook<ReturnType<typeof _useGetTracks>>(_useGetTracks);
