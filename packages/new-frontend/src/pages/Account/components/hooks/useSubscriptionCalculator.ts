...
const _useSubscriptionCalculator = () => {
  ...
  const [{data: dataPurchase, loading: loadingPurchase, error: errorPurchase}, fetchPurchase] = useLazyFetch({
    url: `${BACKEND_URL}/subscription/purchase`,
    method: 'post',
    cache: false
  });

  const [{data: dataAddArtist, loading: loadingAddArtist, error: errorAddArtist}, fetchAddArtist] = useLazyFetch({
  ...
  return {
    openAddSubscription, setOpenAddSubscription,
    data,
    dataPurchase,
    plan,
    openModal,
...
export const useSubscriptionCalculator = () => useSharedHook<ReturnType<typeof _useSubscriptionCalculator>>(_useSubscriptionCalculator);
