...
import {ReactCurrentDispatcher} from './lib/react-shared-internals.js';
import {useForceUpdate} from './lib/use-force-update.js';
...
const externalDispatcher = (originDispatcher, pointer) => {
  let boxIndex = pointer ?? 0;
  return {
    ...
    useEffect(fn: any, deps: any[]) {
      ...
      const box = getBox(boxIndex);
      boxIndex++;
      // console.log(p, pointer, 'useEffect');
      if (!box.initialized) {
        // throw new Error('useEffect');
        console.error('useEffect', fn, deps);
      }/* else if (shouldUpdate(box.deps, deps)) {
      console.warn('useEffect.deps');
      // box.deps = deps;
