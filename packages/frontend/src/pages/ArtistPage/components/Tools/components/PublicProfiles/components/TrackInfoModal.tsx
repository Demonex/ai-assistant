...
const TrackInfoModal = memo(({setOpenTrackInfoModal, openTrackInfoModal}: any, any) => {
 ...
 const isEven = (number) => {
  ...
  return (
    <Transition appear show={openTrackInfoModal} as={Fragment}>
      <Dialog as="div" className="fixed inset-0 z-40 overflow-y-auto" onClose={() => setOpenTrackInfoModal(false)}>
        <Transition.Child
          as={Fragment}
        ...
        <div className="h-full">
          ...
          >
            <Dialog.Panel
              className=" bg-popup_gray lg:my-20 p-4 md:py-8 lg:py-10 md:px-8 lg:px-[3.75rem] lg:mx-auto lg:rounded-[20px] z-50 relative w-full lg:max-w-[60rem] mb-[4.5rem] lg:mb-[unset] h-full lg:h-[unset]"
            >
              <div className='w-full flex justify-end cursor-pointer' onClick={() => setOpenTrackInfoModal(false)}>
                <SecondaryCloseIcon className='stroke-white'/>
              </div>
              <div className='mt-6 flex flex-col gap-6'>
                <div className='py-2 flex flex-col md:flex-row gap-6 md:items-center'>
                  <div>
                    <img alt='' src={popupData.popupInfo.avatar} className='w-20 h-20 rounded-xl'/>
                  </div>
                  <div className='flex flex-col gap-2'>
                    <h1 className='text-t1Semi_deck'>{popupData.popupInfo.trackName}</h1>
                    {
                      ...
                      <defs>
                        <linearGradient id="Color0" x1="0" y1="0" x2="0" y2="1">
                          <stop offset="0%"
                                ...
                                stopOpacity={1}/>
                          <stop offset="100%"
                                stopColor={popupData.popupInfo.chartData.secondaryColor}
                                stopOpacity={0.2}/>
                        </linearGradient>
                ...
                <div className='w-full flex justify-center items-center gap-4'>
                  <p className='text-caption_s_desk text-medium_grey '>
                    *компания Meta Platforms Inc., владеющая Facebook и Instagram, внесена в реестр экстремистских
                    организаций,<br/> ее деятельность в России по поддержанию указанных соцсетей признана
                    экстремистской
                    деятельностью
                  </p>
