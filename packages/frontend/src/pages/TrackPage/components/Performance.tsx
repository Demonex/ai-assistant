          ...
          : <>
            <div className=" flex flex-col items-center gap-6 w-full justify-center lg:px-7  py-4 lg:py-8 ">
              <div className='w-full flex justify-between items-center pb-4 border-b border-secondary_dark_gray'>
                <p
                  className="text-btnText text-light_grey">Статистика</p>
...

              {
                chartTrackData?.chart.iconData.map((item, index) => {
                  return (
                    <div className="w-full overflow-y-scroll flex flex-col gap-3 py-2" key={index}>
                      <div
                        className="text-white w-full"
                      >
                        <div className="flex justify-between ">
                            <p
                              className="text-caption_r_desk text-medium_grey">{item.text}</p>
                          <p
                            className=" text-t1Semi_deck ">{item.count}</p>
                        </div>
                      </div>
                    </div>
                  );
