...
const Team = () => {
  ...
  return (
    <section className='py-6 flex flex-col gap-6'>
            ...
            teamMembers.map((item, indexItem) => (
              <tr className={``} key={indexItem}>
                ...
                <td
                  ...
                  <div className='flex justify-end pr-8 items-center' onClick={() => setOpenModalRemove(true)}><BasketIcon
                    className='fill-white hover:fill-medium_grey cursor-pointer'/></div>)}
                </td>
              </tr>
            ))
          }
      ...
      <SecondaryButton title='Добавить участника' className='border-none bg-primary_blue text-white w-fit'
