...
export const DropdownAccountMenu = memo(() => {
  ...
  return window.document ? createPortal(
    <Menu as="div"
      ...
      {({open}) => {
        return (
          <>
            ...
            >
              <Menu.Items
                className="mt-[50px] w-56 origin-top-right rounded-md  bg-[#15172300]/20 border border-[#ffff]/10 backdrop-blur-2xl shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                <div className="w-full h-full "/>
                <div className="px-4 py-3 border-b border-gray-100/5">
                  <p className="text-sm text-gray-300">Signed in as</p>
                  <p className="truncate text-sm font-medium text-gray-100">{profile?.email}</p>
                </div>
