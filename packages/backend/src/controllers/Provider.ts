...
export class ProviderController {
	...
	async handleOauthCallback(
		...
		switch (provider) {
			case "apple": {
				try {
					const { id_token } =
						await this.providerService.appleGetAccessTokenIos(code);
					...
					await this.providerService.authByProvider(
						`APPLE_${_json.sub}`,
						user,
						true,
					);
					const sid = encodeURIComponent(
						`s:${signature.sign(res.req.sessionID, process.env.COOKIE_SECRET)}`,
					);
					console.log("sid", sid);
					return res.send({
						sid,
				...
				return passport.authenticate(provider, { state: code }, (err, user) => {
					...
					if (!user)
						res.redirect(
							...
							`${process.env.FRONTEND_URL}/error?code=auth-social`,
						);
					res.redirect(
						HttpStatus.SEE_OTHER,
						`${process.env.FRONTEND_URL}/user/social`,
					);
				})(req, res, next);
			}
		}
	...
	async handleOauthCallbackPost(
		...
		try {
			...
			await this.providerService.authByProvider(`APPLE_${_json.sub}`, user);
		} catch (e) {
			console.error(e);
		}
		return end();
