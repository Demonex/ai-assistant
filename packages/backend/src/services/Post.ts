...
export class PostService {
	...
	async getPost(id: string, language): Promise<any> {
		...
		return {
			...
			description: item.description[language],
			content: serialize(
				item.content[language]?.root?.children,
				await this._getMedia(item.content[language]?.root?.children),
			),
		};
	}
	...
	private async _getMedia(
		...
		const ids = children.reduce((prev, node) => {
			return node.type === "upload" && node.relationTo === "post-media"
				? [...prev, node.value?.id].filter(Boolean)
				: prev;
		}, []);
		return (
			await this.repoPostMedia.find({
				_id: { $in: ids },
			})
