...
const getHeaders = () => ({
	...
	"x-enigma": String(420 * Date.now() + 69)
		.split("")
		.map((e) => e.split("").map((e) => e.charCodeAt(0)))
		.map((e) =>
			((x) => x.split("").map((_) => _.charCodeAt(0)))(
				"<img src='/getbaited.jpg' />",
...
export class ProxyService {
	constructor(
		...
		@InjectModel(LogEntity)
		private readonly repoLog: ReturnModelType<typeof LogEntity>,
		@InjectModel(SubscriptionEntity)
		private readonly repoSubscriptions: ReturnModelType<
	...
	async isSubscribed(user: Types.ObjectId, artist?: string) {
		...
		return Boolean(
			await this.repoSubscriptions.exists({
				user,
				artists: artist,
				archived: { $ne: false },
			}),
