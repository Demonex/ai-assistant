...
export class SubscriptionService {
	constructor(
		@InjectModel(SubscriptionEntity)
		private readonly repo: ReturnModelType<typeof SubscriptionEntity>,
		@InjectModel(SubscriptionPlanEntity)
		private readonly repoPlans: ReturnModelType<typeof SubscriptionPlanEntity>,
		@InjectModel(SubscriptionTransactionEntity)
		private readonly repoTransactions: ReturnModelType<
			typeof SubscriptionTransactionEntity
		>,
	) {}
	...
	async findByUser(user?: Types.ObjectId): Promise<any> {
		return this.repo
			.find({ user })
			.select([
				...SubscriptionEntityDefaultSelect.filter((_) => _ !== "user"),
				"createdAt",
			]);
	}
  ...
  async unsubscribe(user?: Types.ObjectId, artist?: string): Promise<any> {
	...
	async purchaseCallback(
		...
		const transaction = await this.repoTransactions
			.findOne({
